...
python3 -m pytest tests/ -q -n 3 --dist loadfile   # parallel, pytest-xdist  (measured 23:30 on 4 cores, 2026-08-25)
                             ...
                             #        smelter mill mill-direct aurora \
                             #        camera_follow \
...
- **The routine contract: an exported routine DECLARES the machine state it
  ...
  **On the feature layer it is REQUIRED; everywhere else it is offered.** The
  grammar landed optional — a file with an old-style prose header was simply
  UNDECLARED, no finding and no baseline — which is what let it arrive on a
  tree whose width-lint baseline is zero. That migration is finished under
  `engine/features/**`, so there the lint now asks for it: a uniquely-named
  routine another file `jsr`s into, with no `CONTRACT` block, is
  `contract-missing`. The flip cost nothing to make because it was already
  true (276 exported feature routines, all declared) — it is a ratchet against
  the NEXT export, not a migration. Outside that scope — a rail's own scene
  routines, the per-rail hook families — and for any name several files define,
  the declaration stays offered rather than demanded, and those callers stay as
  unchecked as they were. A malformed declaration IS a finding wherever it
  sits, because a header that reads as a checked contract while nothing checks
