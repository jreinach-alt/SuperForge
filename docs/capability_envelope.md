...
| `meteor_event` | Mid-level Mode 1 ⇄ Mode 7 cutscene: play freezes, BG platforms are captured into a declared 40-slot OAM claim, the meteor grows about a pinned pivot with a tumble, then the level is restored. Both swaps are SEAMLESS — no displayed frame is blanked; the images are boot uploads and the cut holds its forced blank inside one VBlank. |
...
| `smelter` | Per-column vertical scroll for **zero HDMA channels**: in modes 2/4/6 the PPU reads BG3's tilemap as one scroll offset per 8-pixel column instead of as tiles, so the displacement rides the tilemap fetch a layer already pays for. Four plates on four harmonics over a melt that erupts column by column and CHURNS while it does (an 8-frame CHR swap under an unchanging tilemap) while a band of light drifts left to right across the wall behind it (a colour rotation over eight palette entries, 16 B a frame — the wall carries no pattern in its pixels at all). Both are motion the offset table has nothing to do with, shown apart from it by holding the flat control and watching them keep going; and WHICH mechanism each surface may have is decided by the constraint, not by taste — the wall must stay invariant under vertical displacement, and a palette cycle does not touch a pixel, with a knight standing on a plate and riding it — his collision reads the same word the picture is drawn from, which is what makes the offset a POSITION rather than a display trick. One 64 B VBlank transfer a frame, and 32 independent columns cost what 1 would. Debuts the `[[claims.video]]` / `[[claims.offset]]` vocabulary and the mode-dependent refusal set (`docs/100`), and generalises the per-scene disarm rule from a port to a whole LAYER'S IDENTITY. |
...
| **Per-column layer displacement, for no channel** — offset-per-tile: a BG's tilemap read as scroll data | `smelter` — every gap column's crust line and every plate column's top edge asserted at exactly `map row - word - 1` against the word the ROM holds for that column, on BOTH layers out of ONE table (`tests/test_smelter.py`). Three hardware rules measured on the binary: a column's word displaces the column to its RIGHT; screen column 0 cannot be displaced at all; and a layer's own `BGnVOFS` is the value for EVERY column whose word drives the other layer, so the fallback port is a whole-screen quantity and not column 0's (`docs/100` §12.1 — the defect that taught it, and the trade it forced). **And a player stands on it**: the knight's feet, read off the rendered frame, on the plate's top edge, read off the same frame, at the height the ROM's own word puts it — through a jump, a fall out of the world, a death by lava and the flat control. **And it SCROLLS**: four screens off one world-space table, the camera moving the DMA's read head rather than rebuilding anything, asserted at a camera two screens in | `smt_opt` + `smt_rom` + `smt_bg`; the vocabulary is `[[claims.video]]` / `[[claims.offset]]` and the allocator holds one mode per scene and refuses the table outside modes 2/4/6 — refusal set O1–O8, `docs/100` §5. **And the table can be read in BANDS**: `mill`'s melt reads three rows of it in one frame — the machines from the hall's row, the deck from a row with no enable bit, the channel from a ripple row — through `[[claims.offset_bands]]`, the composition's own HDMA channel on BG3VOFS (O10, `docs/100` §13.7); the same screen column carries a horizontal word in one band and a vertical one in another, asserted off the picture |
...
| **Region parity** — the same ROM at the same REAL-TIME speed on NTSC and PAL, opt-in per rail | 33 of the 40 compose it — every playable rail, plus all three screen-effect rails — and all 33 are measured, at a band of **0.994–1.027** against the **0.832** an uncompensated rail reads (`docs/98` §1). `smelter` measures 0.99989 on `column_phase` AND, since it gained a player, 0.99989 again on `knight_cycle` (his run and his gravity jointly, which the phase's observable could not see) — to five places the same as `heathaze` all three times, because a shared scaler applied to different constants rounds the same way. **A rail that gains a moving character owes a second observable**: a region error in the physics leaves the animation's ratio untouched. The other 7 decline in their own `game.toml`: all determinism trials whose frame-indexed sweeps are the thing under test. NTSC is pixel-identical against every pre-change image | `region` (reads the console's region line once at boot, publishes `ES_RGN_PAL`) + `tick_scale` (the `TS_STEP` macro; no claims of its own, `depends = ["region"]`). Part 3 item 7 is what a consumer still has to decide |
