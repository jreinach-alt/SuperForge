...
| **Region parity** — the same ROM at the same REAL-TIME speed on NTSC and PAL, opt-in per rail | 29 of the 37 compose it — every playable rail — at a measured band of **0.994–1.027** against the **0.832** an uncompensated rail reads (`docs/98` §1). The other 8 decline in their own `game.toml`: 7 determinism trials whose frame-indexed sweeps are the thing under test, and `rpg`, deferred with the measurement that defers it. NTSC is pixel-identical against every pre-change image | `region` (reads the console's region line once at boot, publishes `ES_RGN_PAL`) + `tick_scale` (the `TS_STEP` macro; no claims of its own, `depends = ["region"]`). Part 3 item 7 is what a consumer still has to decide |
