...
6. **`test_measure_vblank::test_measure_multi_queue_arm_cost`, again** — the
   2026-08-26 landing runs at `715dec0`. The probe wait hit the same 30 s
   frozen-counter guard (`vendor/mesen_runner.py:3767`), and the module passes
   alone in 1.91 s against a diff that touches no measurement code. Instance 4
   fixed the *starvation* path that produced it once; this is the residual
   flake the `Makefile` `gates:` comment already prices at **4 pass / 1 fail
   over 5 invocations** for the legacy free-running runner (`Makefile:2842`).
   Not fixed here — the fix is migrating `measure` off that runner, docs/53
   wave 2. What changed is that the artifact now *says* it is this (§8).
7. **`test_sprite_game::test_the_whole_screen_is_two_sprites_over_bare_backdrop`**
   — the second run of the same tip. `machine.MachineError: close(): the core
   did not resume free-running`, raised in an autouse fixture's **teardown**,
   after the test itself passed. `Machine.close()` verifies against emulated
   progress but bounds the wait at `time.time() + 10.0`
   (`vendor/machine.py:651`), so its false-positive rate is a function of host
   load rather than of the tree. Also unfixed, also now named by the artifact.

Instances 6 and 7 are why §8 exists: both runs recorded **2211 passed, 15
skipped, zero test failures** and identical ROM md5s, and each run's red step
was green in the other — a picture that took twenty minutes to assemble by
hand and reads in one line once the fault is classified.
...
## 8. Reading a red: what KIND of fault it was

§4 is a list of seven times the verdict bit was set by the host rather than by
the tree. Until 2026-08-26 the artifact could not tell the two apart: `suite`
held pytest's summary *line*, `suite_schedule` (§7's sibling) said who ran next
to whom, and nothing said what the fault actually **was**. So `2211 passed …
1 error` and a genuine assertion failure produced the same RED and the same
forensic dig.

`tools/harness_faults.py` parses the `FAILURES` / `ERRORS` blocks out of the
suite log and the tee'd gate log, and labels each fault:

| reading | means |
|---|---|
| `defect` | anything the tree is answerable for — the default |
| `harness-liveness` | the raised message is one of the **wall-clock liveness guards** in `vendor/machine.py` / `vendor/mesen_runner.py` |

`bare_check.json` gains `faults` (test, file, phase, exceptions, reading) and
`fault_reading` (`none` / `defect` / `harness-liveness` / `mixed`); the closing
summary prints them beside the red-module neighbours §7 added.

The two records answer different questions and are both needed. `suite_schedule`
answers *who could have poisoned this module* — the pair-hunt. `faults` answers
*was it poisoned at all, or did a rail break*. Reaching for the pair-hunt on a
fault that was never a defect is most of the twenty minutes.

**It does not move the verdict.** `verdict` and `exit_code` are untouched, a
`harness-liveness` reading is never a pass, and a second sighting of one is a
bug report — against the harness rather than a rail. What it buys is a report
legible in one glance instead of one turn.

Three things make it hard to rubber-stamp:

- **The guard table cannot drift.** Each row names the vendor file its fragment
  comes from, and `tests/test_harness_faults.py` asserts the fragment is still
  there. A reworded guard fails that test rather than quietly reclassifying
  every future fault as a defect. Proved by planting a rewording: the drift
  test goes red, *and* so does the classification test.
- **Only `E ` lines count.** The same text appears in traceback *source* lines
  whenever the raising frame is displayed — it is, in the wedge fixture — so
  matching the whole block would read any test that merely prints a guard as a
  harness fault.
- **The guards' own test modules are exempt** (`test_park_guard`,
  `test_wait_primitives`, `test_machine*`). They assert *on* the guard text by
  design; a red in one of them is a red in the guard, which is a defect.

The fixtures are verbatim output from the two runs §4 items 6 and 7 describe,
plus a negative control generated by running two genuinely failing tests. None
is hand-written, so the tool cannot pass by recognising a shape drawn for it.
The wedge fixture is the case a naive rule reads backwards: its surface
exception is an `AssertionError` ("probe never answered"), with the
`TimeoutError` only in the *During handling of the above exception* chain.

**A second defect, found while wiring this.** `make test` and the determinism
gate passed `-rs`, and an explicit `-r` **replaces** pytest's default set
(`fE`) rather than adding to it — so the short summary had stopped naming
failures and errors entirely. §4 instance 7 appeared in `build/pytest.log`
only as an `ERRORS` block far above the tail and a bare `1 error` in the
count; the 20 lines the recipe prints named nothing at all. Now `-rsfE`.

And `step()` in `tools/bare_check.sh` now tees. `make measure` runs a pytest of
its own straight to the console and writes no log, so before this its faults
existed nowhere on disk — the artifact could only ever say `measure FAILED`.
...
- `tools/harness_faults.py` — the fault classifier and its guard table;
  `tests/test_harness_faults.py` proves it against the real faults.
