...
| `vendor/art/forge_line/` (3), `vendor/art/the_end/` (1), `vendor/art/buried_machine/` (7) | **first-party, ON A STATEMENT** | generated by the project owner with ChatGPT and supplied to the build session. These are the ONLY rows in this audit reaching none of §1's evidence classes for their ORIGIN; their bytes are fixed, and the derivation downstream is reproducible for the first two — the third is staged art nothing in the build reaches yet. §5.1a, §5.1b and §5.1c state it in full |
...
### 5.1c `vendor/art/buried_machine` — the third row that rests on a statement

Seven asset sheets for an unbuilt `tiles16` platformer concept, **generated
by the project owner with ChatGPT and supplied on 2026-09-08** in two
deliveries. Same situation as
§5.1a and §5.1b, recorded separately because a reader looking for one pack
should not have to know about the other two.

| evidence class | available here? |
|---|---|
| regeneration | **no** — no generator in this tree emits them, and a prompt is not a reproducible one |
| byte-match to an upstream artifact | **no** — there is no pack archive and no upstream copy to match against |
| the upstream's own grant, fetched | **n/a** — there is no third party to grant anything |
| structural reachability | n/a — nothing in the build reaches them yet |

**Nothing in the tree consumes these bytes today.** They are staged art for a
concept that has not been built, so unlike §5.1a there is not yet a
deterministic derivation resting on them — which makes this row weaker in
reach but no weaker in kind. `vendor/art/buried_machine/SHA256SUMS` fixes the
seven files as supplied, and the pack README carries the same origin caveat at
the top of the file.

What IS established is what the sheets ARE, measured rather than assumed and
reproducible from the bytes: 540 separable assets across the seven; 51k-152k
unique colours per sheet; **0.00 % of substantially-opaque aligned 8x8 blocks
constant on every one of them**; and, on `sandstone_ruins.png`, an
integer-upscale probe at chance for every N in 2..12. They are not pixel art,
they cannot be sliced, and the conversion path is `tools/kit_import.py`'s
resample — the §5.1a finding, reproduced on a second pack.

The ORIGIN, as in §5.1a and §5.1b, is the owner's statement, and they are
recorded as first-party on that basis.
