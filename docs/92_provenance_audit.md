...
| `vendor/art/forge_line/` (3), `vendor/art/the_end/` (1) | **first-party, ON A STATEMENT** | generated by the project owner with ChatGPT and supplied to the build session. These are the ONLY rows in this audit reaching none of §1's evidence classes for their ORIGIN; their derivation downstream is reproducible and their bytes are fixed. §5.1a and §5.1b state it in full |
...
### 5.1a The `forge_line` concept sheets — the one row that rests on a statement

`vendor/art/forge_line/` holds three chroma-keyed asset sheets for the `mill`
rail, **generated by the project owner with ChatGPT and supplied to the build
session on 2026-08-29**. They are the only vendored art in this tree whose
provenance reaches NONE of §1's four evidence classes, and this section exists
to say so rather than to let a README stand in for a trace.

| evidence class | available here? |
|---|---|
| regeneration | **no** — the sheets are not the output of any generator in this tree, and a prompt is not a reproducible one |
| byte-match to an upstream artifact | **no** — there is no pack archive and no upstream copy to match against |
| the upstream's own grant, fetched | **no** — generated art ships no licence file |
| structural reachability | n/a |

What IS established: `vendor/art/forge_line/SHA256SUMS` fixes the three files
as supplied, and everything the build derives from them is the deterministic
output of `tools/kit_import.py` + `tools/gen_mill_assets.py` over exactly
those bytes. So the DERIVATION is reproducible; the ORIGIN is the owner's
statement.

That is the honest verdict and it is a weaker one than every other row in this
audit. §1's trap is a licence its own author wrote; this is one step further
out — a README this audit's own author wrote, from what he was told. It is
recorded as first-party on that basis, and the pack's README carries the same
caveat at the top so a reader meets it before the file list rather than after.

### 5.1b `vendor/art/the_end` — the second row that rests on a statement

`vendor/art/the_end/the_end_traced_strokes.svg` is the pen path the `aurora`
rail's writing follows: 23 open polylines, a **centreline skeleton** rather
than an outline or a font. **The lettering it traces was generated by ChatGPT
at the project owner's request, and the trace was produced in the same session
and committed by the owner** (2026-09-04).

It is §5.1a's situation exactly, and it is recorded separately rather than
folded into that row because the two were supplied a week apart and a reader
looking for one should not have to know about the other.

| evidence class | available here? |
|---|---|
| regeneration | **no** — no generator in this tree emits it, and a prompt is not a reproducible one |
| byte-match to an upstream artifact | **no** — there is no upstream; it was made for this repo |
| the upstream's own grant, fetched | **n/a** — there is no third party to grant anything |
| structural reachability | **partial** — see below, and it is stronger here than in §5.1a |

What IS established, and one part of it is worth more than §5.1a's: the file
is **not a copy of any published typeface**, and that is checkable rather than
asserted. A typeface ships OUTLINES — closed contours with fills, two edges per
stroke. This ships 23 OPEN polylines with `fill="none"`, one per stroke
fragment, running down the middle of the ink. No font format encodes a
centreline, so no font could have been converted into this without the
conversion being the authoring step. `tools/write_on.py` then recovers a
drawing order from the graph, which is likewise not something a copied artifact
would need.

The DERIVATION downstream is fully reproducible: `tools/write_on.py` and
`tools/gen_aurora_assets.py` are deterministic over exactly these bytes.

sha256 `44d1186fb87004f8e2712a7c8c9f019be7a2f42a8ed7da272cd00071cc08d7c4`

The ORIGIN, as in §5.1a, is the owner's statement. That is the honest verdict,
`vendor/art/the_end/README.md` carries the same caveat at the top of the file,
and it is recorded as first-party on that basis.
