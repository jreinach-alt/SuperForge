...
| `aur_roll` | **feature** | unused | `dp`&times;2, `dma_init`, `dma` | `aur_rom`, `aur_bg` |
...
| `aur_roll` | **THE PALETTE FIELD AS A LIVE PER-TILE COLOUR CONTROL, which is the whole argument for building a picture on direct colour rather than merely rendering one with the bit set.** In an indexed 8bpp layer the tilemap entry's three palette bits are ignored outright (":1077 Ignore palette bits for 256-color layers"); under direct colour they carry the LOW BIT OF EACH CHANNEL, so a map word is a colour control and a wave of light travels up 143 curtain tiles for one word per tile — no CHR traffic, no CGRAM, no palette to fit. **THE MASK IS OR-ED, NOT XOR-ED**, and that is the difference between a wave and a rash: tiles are fitted to whichever field suits their own pixels, so adjacent tiles commonly sit on different ones, and XOR-ing the same bit into both SWAPS them — one rises a lattice step while its neighbour falls, and the curtain breaks into red and blue eight-pixel blocks. Measured on the first cut. **AND THE BLUE BIT IS NOT LIKE THE OTHER TWO**: the pixel gives blue only two bits, so its field bit is worth 4 of 31 against 2 elsewhere — 33 units of 255 against 16 — which is why blue is gated to the curtain cores. All 32 phases are precomputed and a phase is ONE VBlank transfer; 32 rather than 64 because 64 pages is 53,248 B and a DMA source cannot span a bank |
