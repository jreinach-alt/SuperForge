...
**All 177 dirs under `engine/features/` accounted for.** 89 feature &middot; 48 blob &middot; 3 companion &middot; 27 consumer &middot; 10 game_logic.
...
| `mil_melt` | **feature** | unused | `wram` | `mil_opt` |
...
| `mil_melt` | **THE OFFSET TABLE READ IN BANDS** — the `mill` rail's third room reads THREE rows of `mil_opt`'s table in one frame, because the PPU fetches the row BG3VOFS names (`rowOffset = VScroll >> 3`, SnesPpu.cpp:257-276) and an HDMA channel rewrites that port at each band's first line: the machines and the car from the hall's running row, the deck from a row with no enable bit (the control, in the picture itself), the channel from a RIPPLE row where every column carries a vertical word — the same screen columns that carry a HORIZONTAL word two bands up. Mode 4 makes the axis a per-column choice; bands make it per-column PER BAND, and the case reads both off one pair of frames. The declaration is `[[claims.offset_bands]] rows = 3` — a claim of the SCENE's use of the table, on a scene-scoped feature beside it rather than a field on the global table claim, because only this room reads it so. The composition SYNTHESIZES the channel (`mil_bands_rowsel`: BG3VOFS, mode 2, active, the whole frame), assigns and emits it, and marks its own BG3VOFS ownership `seed`, so a second active channel on that port meets it as an ordinary HDMA register contention (O10) — the refusal a raw `seed` beside a raw hdma claim could not reach. This feature arms the channel through the scene_mgr shadow and seeds its line counter to a plain hold, because a channel enabled in VBlank runs on that VBlank's remaining lines from whatever `$43xA` holds and a zero there wraps to "repeat, 127 lines" — measured as 382 reads of never-written RAM on the room's first frame before the seed, none after (docs/100 §13.7) |
