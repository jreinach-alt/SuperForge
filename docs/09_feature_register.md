...
composed color-math vocabulary, docs/99). Before this file, the
...
**All 166 dirs under `engine/features/` accounted for.** 82 feature &middot; 46 blob &middot; 3 companion &middot; 25 consumer &middot; 10 game_logic.
...
| `haze` | **feature** | unused | `vram`&times;2, `cgram`, `dp`&times;2, `hdma`&times;2, `dma_init`, `reg`&times;2, `screen`, `blend` | `hz_rom` |
...
| `hz_rom` | **feature** | unused | `rom`&times;7 | &mdash; |
...
| `haze` | **DISPLACEMENT + BLEND** — heat shimmer as a per-scanline `BG1HOFS`, delivered by one HDMA channel across a declared band, plus a BG2 glare layer on the SUB screen half-added onto the world by the PPU's one colour-math unit. **The displacement and the blend are one feature, and that is the composition**: `hz_bg` designates bg1 and bg3 to the main screen and knows nothing about either half, this feature designates bg2 to the sub screen, declares `[[claims.blend]]` add/half/source=sub/math=[bg1], and separately owns TWO HDMA channels — `hzwarp` on BG1HOFS and `hzshim` on BG2HOFS, reading the SAME table `HZ_SHIM_LEAD` phases apart, so the glare slides across a surface that is itself moving rather than riding it like a decal. `backdrop` is absent from `math` deliberately (it is this rail's sky, above the band, and admitting it would glare the sky — the reference sheet's "use it locally, only near sources" refused at the declaration instead of hoped for in the art), and `bg3` is absent so text stays legible. **The warp is a TABLE, not artwork**: authoring pre-warped copies of the affected tiles doubles the tile budget, distorts only what was drawn in advance, and cannot follow the art it bends — the PPU bends a whole layer natively and for free during active display. `hz_rom` holds 32 complete HDMA tables at a 256 B stride plus a 33rd all-zero CONTROL, so a blob's address is `hz_warp + (n << 8)` and the entire per-frame cost is ONE 8-bit store to the channel's A1T high byte; rebuilding the table instead is priced by `platformer_bg` at ~16 cycles an entry. Amplitude is baked in as a function of SCREEN y (zero at the horizon, widest at the viewer's feet) while phase indexes the blob, so advancing the index scrolls the wave UPWARD through a ramp that stays pinned to the screen. It claims `BG1HOFS` TWICE — an `hdma` claim for the band and a `seed` `reg` claim for the base the head-skip entry restates — which the allocator requires to be a matched pair (`check_reg_ownership`, checks 2 and 3). The phase advance is a velocity and takes one factor of r through `TS_STEP` |
| `hz_bg` | **BG** — the `heathaze` rail's desert world AND its display shape: BG1's road-to-a-mesa picture (a four-step Bayer sky, eight sub-cell ridge profiles so the skyline is smooth rather than a comb, a bleached horizon strip, and a floor scattered by a hash rather than a modulus) plus BGMODE $09, BG1SC, BG12NBA and BG1VOFS. Vocabulary-native like `lake_bg`, and for the same reason — it designates bg1 and bg3 with `[[claims.screen]]`, so `bg_text` composes untouched. **`BG1HOFS` is deliberately NOT here**: on this rail that port belongs to whichever feature answers for it in the scene, `haze` in the desert and `hz_flat` on the title, because one is a seed under a transfer claim and the other is a plain owner and a feature cannot conditionally claim |
| `hz_flat` | **The composed UNDISTORTED state for `BG1HOFS`, as a claim** — `blend_off`'s shape on a register that is not the blender, and the generalisation this rail paid for. `haze` drives `BG1HOFS` per scanline, so at a scene edge the port holds whatever the last scanline left in it; a successor composing no `BG1HOFS` claimant writes nothing and inherits a displaced world. Compose this into a non-shimmering scene and that scene owns the port, gets the symbol emitted, and establishes the flat base on enter. **An HDMA-driven register needs the same per-scene disarm discipline the blender does** — colour math was never special, it was the first port anyone noticed it on |
