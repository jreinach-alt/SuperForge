...
| **AUD** | `audio` (deps: `tad_rom` blob) | `spc` (whole-space exclusive) + `reg` (`APUIO`) + `wram` (16 B, pinned `$1F00` — the driver's linker-placed `.bss`) + `dp` (2 B, pinned `$F0` — the SFX queue) ; `tad_rom`: `rom` (16 KB half-window pinned to window 1, bank-start by arithmetic) | — | `boss_saucer`, `brawler`, `breaker`, `camera_follow`, `heathaze`, `hud_game`, `jumper`, `lakeside`, `m7_dungeon`, `m7_oshoot`, `maze`, `microzero`, `mill`, `mode7_explore`, `mode7_flight`, `patrol`, `platformer`, `platformer_stream`, `racer`, `railshooter`, `room`, `rpg`, `scroll_run`, `shmup`, `smelter`, `split_v_fight`, `sprite_game`, `stomper` — **28 of 42, and NOT re-derived by any gate** (`make register` checks the census, not this list; `make register-write` leaves it alone — so it is prose, and stale the day a rail is added without editing it). It has gone stale three times — the third while this very branch sat unmerged and `aurora` landed on main, taking the denominator from 41 to 42 without touching a line of it: once naming three rails that did not compose `audio` and omitting three that did, and once sitting at 17 through a pass that took it to 28. **Do not hand-maintain it — derive it**, which is one line and is how the list above was rebuilt: `python3 -c "import pathlib,tomllib; print([p.parent.name for p in sorted(pathlib.Path('game').glob('*/game.toml')) if 'audio' in set(tomllib.loads(p.read_text()).get('globals',[]))])"` | 3 | **built** (2026-07-30) — vendored TAD at `822164b` (`vendor/tad/`), content pipeline documented in `assets/audio/README.md` (procedural samples, checked-in ca65-export). The `room` game ships it: one song persisting across three scenes, per-room reverb (EVOL/EFB against a program-constant EDL — erratum), a footstep under music. Surfaces: `tests/test_slice_b_audio.py` (SPC-RAM boot compare, DSP A→B→A cycle, tick-counter persistence, WAV audibility + aligned rest-tail difference) and `tests/test_sfx_vocabulary.py` (SFX voice keyed + ENVX sounding, positional pan in both directions, cue cadence), with `tools/plants/sfx_vocabulary.py` as the 5-plant falsification set. **CONTENT (2026-09-08): 3 sound effects → 14.** The set is shared across all eight rails because the export blob is one artifact, and costs 251 B (8,450 → 8,701 of a 16,384 B claim) because the S-DSP's noise generator and pitch sweeps carry it — only two new instruments, both single-cycle loops. Two hardware facts the content pass had to find on the emulator, both invisible to the compiler: a DECREASE-mode GAIN (`E<rate>`) falls from the envelope's current level and key-on leaves that at zero, so such an effect keys on and stays at ENVX 0 forever; and **TAD's default audio mode is MONO** (`tad-audio.inc:123`), which collapses every channel to centre, so panning requires `Tad_audioMode` set between `Tad_Init` and `Tad_LoadSong`. **CLAIM (2026-09-08): 32,768 → 16,384 B.** The export is 8,752 B of segment content, so a whole window reserved 24,016 B nothing could use — and every audio rail carried its own copy. `BANK1` gained `align = $4000` in `lorom_512k.cfg` so ld65 starts it where the allocator's arithmetic says (the export's real content is shorter than the claim); a rail without audio is unaffected and every one measured byte-identical, `microzero`'s pinned md5 included. Measured: each audio rail sheds 16,384 B and seven of eight drop a window (`room`/`breaker`/`platformer`/`shmup`/`split_v_fight` 2→1, `boss_saucer` 3→2, `racer` 14→13). Cart bytes followed only where a rail could then be RELINKED: `lorom_512k.cfg` fills all sixteen banks whatever the allocator uses, so the claim change alone bought headroom rather than size. The five rails whose whole allocation fitted ONE window (`room`, `breaker`, `platformer`, `shmup`, `split_v_fight`) were moved to `lorom_64k.cfg` — which gained `AUDIO_DATA0`, the same `align = $4000`, and the pinned `TADZP`/`TADBSS` windows — and are now **65,536 B**, declaring `$06` with no source change (header.inc imports `SF_LD_ROM_SIZE` from the config). `boss_saucer`, `racer` and `rpg` still span more than one window and stay at 524,288 B. Settlement unchanged: TAD's compiler packs the interior; SuperForge owns the boundary. **PHASE 2 (2026-09-09): 8 rails → 17.** `jumper`, `stomper`, `sprite_game` and `patrol` composed `audio` + `tad_rom`, each reversing a NOT-COMPOSED line in its own game.toml that said the rail was "silent" — a statement about the rail, never about the machine. All four are on `lorom_512k.cfg`, which already carries `AUDIO_DATA0`, so the 16 KB claim costs no size class: 524,288 B before and after. The cues are the events each game already funnels through one site. TWO SHAPES, and the distinction is the whole of the work: an event that is one-shot BY CONSTRUCTION needs no edge (sprite_game's catch teleports the dot on the frame it fires; patrol's knockback moves the player off the enemy; stomper's stomp kills it), while a cue read off a STATE does — both writers of US_GROUNDED run on every frame the box rests on floor, so `jumper`/`stomper` route them through a `*_ground` helper that tests the transition. Planted: removing that edge test takes the thud voice from 8% of frames to 51% on jumper and 35% on stomper, against a 25% bar, and BOTH audibility cases still pass on the plant — which is why the cadence case exists. Surfaces: `tests/test_jumper_audio.py`, `tests/test_stomper_audio.py`, `tests/test_small_rail_audio.py`, every fixture asserting the game's own counter moved (US_FOES / US_SCORE / US_HITS) so a rotted drive fails on the counter instead of going quiet on the cue. `maze` closed the pass's one deferral and needed a THIRD shape: free pixel movement has no step commit to hang a footstep on, and the rpg's overworld says as much where it declines to sound a wall knock — "needs an edge latch, and this rail has no state for one". So the rail declares one: `cell`, the 16 px stride the player was in last tick, and the cue is its CHANGE. Sixteen and not the map's eight because of the ear — at MZ_SPEED = 2 px/frame an 8 px stride put the step voice on 63% of walking frames, which is no cadence anyone walks at; 16 px gives 41%, about four steps a second. Three drives assert it: walking sounds, idle is EXACTLY silent (planted, a per-frame cue reads 86% with no input), and shoving into a wall never steps faster than a free walk — that last one deliberately not asserted silent, because this rail SLIDES along a wall by design and should keep stepping while it travels. `brawler` added the case where TWO CUES SHARE A SAMPLE: the landed swing takes `hit` and the damage taken takes `thud`, both voiced by `step`, so SRCN cannot tell dealing a blow from receiving one. What separates them is that `hit` opens with a noise transient and `thud` does not — audible, and readable on the chip as the voice appearing in NON, since nothing else on this rail takes the noise generator. Planted by swapping the swing's cue to `thud`, a change INVISIBLE to every SRCN assertion, that case fires alone. The KO takes `chime` (bell) precisely so a win is not a third thump. brawler is also the first rail the claim pushed across a WINDOW: its blobs were 17,682 B, tad_rom's 16,384 takes them to 34,066, and the smallest blob (the font) spilled into window 2 — the link refused the build by name until the source said so, which is the `.incbin` asserts working as designed. `m7_oshoot` is the first rail whose stated reason had simply EXPIRED — it declined audio because "adding a soundtrack would be content it does not have", which was true when written and which the Phase 1 pass made false. Its gun cue sits BELOW `do_fire`'s pool-full bail so a press that spawns no bolt makes no sound, and proving that needed a second drive: the case was VACUOUS under the normal one, because at 8 bolt slots and a 90-frame life the pool never fills at a press every six frames — the plant moved the ROM's md5 and changed nothing observable. Under a drive that presses every SECOND frame the pool saturates and the two separate cleanly, 30% of frames against 88%. The general rule that came out of it is in dx_paper_cuts: a case about a REFUSAL is only meaningful under a drive that provokes the refusal, and a moved md5 says nothing about whether the drive reached the branch. `platformer_stream` settled the question the pass had been avoiding: WHERE A CUE MAY LIVE when the event is computed inside a FEATURE. Its jump and landing are both `pfs_logic`'s — a `role = "game_logic"` feature that owns the player's physics — and queuing there would make the mechanism depend on `audio`, so no rail could take the physics without the soundtrack, and would make the CHOICE of sound the feature's when only "a jump happened" is mechanism. THE SETTLEMENT: a feature publishes state; the game decides whether that state makes a sound. `pfs_logic` is untouched and still silent; the rail declares its first `state.toml` for one word, `gnd`, and reads PL_GROUND's two edges — 1→0 is the take-off, 0→1 the landing, so ONE latch serves both cues. Two plants: firing off the state rather than the edge puts a voice on 239 of 300 idle frames, and updating the latch on one edge only silences the other cue entirely — the second caught by a balance case asserting the two voices alternate, which neither audibility case would notice. The 16 KB claim also took this rail from three ROM windows to FOUR. `camera_follow` takes maze's stride latch unchanged — same 2 px/frame walk, same 16 px stride — and sharpens one case: shoving into the world edge must go EXACTLY silent once the corner is reached, where maze's answer to the same drive is that it keeps stepping. The difference is mechanism rather than taste: maze SLIDES along a wall and keeps travelling, this rail CLAMPS and stops. Planted, a per-frame cue reads 89% idle and 88% cornered. **PHASE 2 CLOSE (2026-09-09): 17 rails -> 19, and the tree's THIRD SONG.** `scroll_run` takes `drive_song` and three cues — the take-off needs no edge (ES_INP_PRESS plus the grounded gate make `do_jump` one-shot by construction), the landing needs one and needs it more than jumper does (THREE writers of US_GROUNDED here, not two: the ground probe, the solid snap and the one-way platform's snap, all routed through `sr_ground`), and the goal chime needs none — which took a plant to establish and is the pass's one real finding. The guard written there was DEAD: `tick` gates on US_STATE at its top and jumps past `goal_check` once won, so the branch could never be taken, and the plant that should have proved it load-bearing PASSED instead. The rule filed for it: a plant that leaves every case green means the guard is dead OR the drive never reached it — decide which before weakening the test. The tail case that survived is falsified by the mistake an author would actually make, a cue on the always-run `@draw` path, at 180 of a 200-frame tail against a bar of zero. `microzero` is the other half, and it moved a PINNED md5 on purpose: the reference game had no soundtrack, and the pin (`tests/test_c2_sram_class.py` plus the two falsify binding tools) exists to catch the SRAM class leaking, not to freeze the ROM — its own docstring says "if this fails and you did not deliberately move microzero". It gets `circuit_song`, written for it: A mixolydian over I - bVII - IV where `drive_song` is D minor, because a racing game wants neither the room's ambience nor a shooter's. Its one cue is a lap chime, and the state it reads was ALREADY the game's — `lap` is declared in microzero's own state.toml and `race_logic` fills it — so this is a milder case than platformer_stream's, where the word being read was the feature's own dp claim. The feature is untouched either way. Planted on the counter instead of its change, the bell reads 97% of the drive. Composing `audio` also flipped this census's own `scope` column for `audio` and `tad_rom` from `unused` to `global`, which is worth knowing about: the column is derived from ONE manifest (`game/microzero/game.toml`), so it read `unused` throughout the pass while seventeen rails composed it. **PHASE 2 COMPLETE (2026-09-09): 19 rails -> 24.** `railshooter`, `mode7_explore`, `m7_dungeon`, `hud_game` and `mill`, and TWO of the five reversed a stated reason that had simply EXPIRED, the same way `m7_oshoot`'s did: railshooter's said "a soundtrack would be content, not mechanism" and mode7_explore's said "adding it is a claim plus the two calls, not a rework" — the second an invitation rather than a refusal, and it cost exactly what it said. THREE OF THE FIVE NEEDED NO NEW STATE AT ALL, which is the pass's finding: `hud_game`'s `bump_score` already gates on ES_INP_PRESS, `mill`'s `@arrived` runs the scene switch two instructions later, and `mode7_explore` slides cell to cell so `m7x_logic` had already published US_LANDED one frame wide for the town trigger. Only `railshooter`'s kill (`scoreprev`) and `m7_dungeon`'s goal (`onwin`) are levels, and both read words the RAIL declares — a milder boundary than platformer_stream's, where the word was the feature's own dp claim. Five plants, one per rail, each restoring to the byte: swapping ES_INP_PRESS for ES_INP_CUR, US_LANDED for US_STEP_ACTIVE, deleting `onwin`'s branch, and moving mill's cue to the per-frame tick (169 of 335 frames against a bar of zero). IN FOUR OF FIVE, EVERY OTHER CASE IN THE MODULE STAYS GREEN ON THE PLANT — an audibility case cannot see a cadence defect, because the defect makes the sound MORE audible. `hud_game` is the exception and the nicer shape: reading the held state scores once a FRAME instead of once a press, so the defect shows from the game state and the chip independently. The 16 KB claim moved bank numbers on four of the five (ld65 refusing by name each time, the `.assert`s working as designed) and `m7_dungeon` needed a blob SPLIT rather than a shift: its 16 KB tilemap now shares window 1 with the export, both being exactly half of one. **AND THE FOUR SCREEN-EFFECT RAILS (2026-09-09): 24 -> 28**, `heathaze`, `lakeside`, `smelter` and `mode7_flight`, for MUSIC ONLY — the absence of cues stated as a declaration rather than an omission, on the argument that a screen effect has no discrete event whose 0 -> 1 edge is a moment, so none of the four imports `sf_sfx_queue_c` and none declares a `prev` word; what they compose is `Tad_Init`, one `Tad_LoadSong` and the per-frame pump. **THAT ARGUMENT SURVIVED ONE DAY AND ONE RAIL** — see the content pass at the end of this row, where re-checking it per member rather than inheriting it found edges already sitting in three of the four ticks. They take `slice_b_song`, the ambient piece, because a drum kit under a screen effect is the "prettify the demo" move that `scroller` and `mode7_chamber` decline FOR THEMSELVES — those two stay silent and their stated reason is still live: they are MEASUREMENT rails and music changes what they measure. `boss` and `meteor_event` stay silent for a different live reason, that their audio half exists as a separate rail (`boss_saucer`), and the nine `split_*`/`seam_*` demos have no game events at all. THIRTEEN OF THE REMAINING SEVENTEEN ARE THEREFORE SILENT ON PURPOSE AND STATED. Because the four queue nothing, their two hardware-rule cases can be EQUALITIES rather than fractions: there is no window in which voices 6/7 are legitimately busy, so any sounding there is a part scored where a future cue would erase it. One module covers all four (`tests/test_screen_effect_audio.py`), parameterised, because they make one claim four times. `railshooter`'s existing suite needed the `BOOT_SKEW` constant `test_stomper.py` paid for first — `Tad_Init` costs four hardware frames and this module's absolutes are counted from the SCENE's first tick — and `rs-probe`, the measurement ROM's second link path, needed the two TAD objects the shipping recipe links. The landing gate caught that one twice over: as a failed step AND as a rom-census that derived 59 images and measured 61, naming `rs_probe` as absent. **CONTENT ON THREE OF THE FOUR (2026-09-09): 3 songs -> 6, 14 effects -> 16, and 28 rails relinked for a change to three.** `heathaze`, `lakeside` and `smelter` each took a song of its own and cues of its own; the rail count did not move, because all three already composed `audio`. **The finding is the one that matters more than the content: the SET'S ARGUMENT WAS DOING THE REASONING.** "A screen effect has no discrete event" was checked against each member instead of inherited, and it held for exactly one — `works` has a B toggle and a Start already gated on `ES_INP_PRESS`, `desert` has a Start, and `lake` has a surf cycle whose crest is a timed moment. Two edges and a clock, sitting in the rails' own ticks the whole time. The argument now covers `mode7_flight` alone, and the general rule is filed in dx_paper_cuts: **a set's stated reason is only as true as its weakest member makes it, and the cheapest place to check is the scene's own tick.** THE SECOND FINDING IS ABOUT THE TEST THAT DID NOT NOTICE. `tests/test_screen_effect_audio.py` asserts, for four rails at once, that voices 6/7 never sound — and it kept passing after all three gained content, because its drive presses nothing and those three boot into a title scene: it was measuring a scene with no cues in it while the content sat one Start press away. **A case parameterised over rails inherits its strength from the weakest drive it runs.** That module is now scoped to `mode7_flight`, the one rail whose premise it still describes, and the three carry the claim in its honest form for a rail with cues — WHAT sounds on 6/7 rather than WHETHER — in `tests/test_{heathaze,lakeside,smelter}_audio.py` (30 cases). THE CONTENT, and each song exists because `slice_b_song` cannot be edited into it: that piece's composed rest half-bar is the window the room rail's reverb A/B is measured in, so borrowing and retuning it would cost another rail its demonstration. `far_ridge_song` is D mixolydian with a flattened SIXTH over a bare fifth drone (a major third is the wide open, a flat sixth is the ache, and the mode is the one that has both); `shallow_water_song` is F major in sevenths and ninths with a dominant that stays suspended, so nothing resolves hard; `foundry_song` is the first in the tree whose subject is RHYTHM rather than harmony — three parts at 42, 32 and 96 ticks whose LCM is 672, so no two repeat their alignment inside a seven-bar cycle. TWO NEW EFFECTS, and both needed a hardware fact that is one CAUTION line in the vendor's docs and was in none of ours: **`play_noise` lasts as long as the INSTRUMENT'S SAMPLE, not as long as you asked.** The noise generator replaces the voice's output but the BRR decoder still hits its END flag, so a non-looping instrument caps the effect at its own length — which is why `skid` gets 96 ms out of `step` and could not get more, and why `wind` (a bed, re-triggered on a cadence) and `wave_break` (three bands slurred under one key-on) are both voiced by `saw`, an instrument whose waveform is never heard and which is chosen entirely for its loop flag. Two agents on two rails made the same wrong copy the same afternoon, because both existing noise effects use `step` and both are short enough that the cap never shows; the reason is now written into `sound-effects.txt` beside the DECREASE-gain trap. COST: the blob went 11,864 -> 13,616 B of a 16,384 B claim, leaving 2,768 B — and **songs are now the growth term, not effects**: the pass added no instrument and no sample and still spent 1,752 B. THE RELINK IS THE PART THAT IS NOT VISIBLE FROM THE DIFF. The export enum is positional, so appending `wave_break` moved `footstep` 12 -> 13 and `skid` 13 -> 14, and every one of the 28 audio rails had to be rebuilt for content that changed on three — a rail linked against a stale object would queue `skid` where it meant `footstep`, and no gate reads an effect's id against its name. `microzero`'s pinned md5 moved for the same reason, on content it does not contain, and its docstring now says to expect that whenever `assets/audio/` changes. **TWO PLAYED-AND-REPORTED DEFECTS (2026-09-09), neither caught by a test, and in both cases the tests were right about what they asserted and asserting the wrong thing.** (1) `microzero` read as having NO sound effects. It had one, the lap chime, firing correctly on a latched edge with a case proving it against a steering oracle — but a lap is upwards of a hundred frames, so the rail was silent for seconds at a stretch and a cue that fires correctly and almost never is indistinguishable from no cue at all. **A cue's CADENCE is part of whether it exists**, and no case asked how often the rail made a sound. The event was already there and unsounded: `race_logic` writes `sector`, the ring's quadrant, four times a lap, and it is the rail's own state word, so the fix is the latch shape the chime already had and touches no feature. A lap is now three checkpoint blips (`select`, pluck) and one chime (bell), the lap arm consuming its own crossing so the chime does not trail a blip by a frame; the START presses that enter and leave get the same confirm. Measured: a cue about every 54 frames. Bounded by a case that counts onsets BETWEEN consecutive lap frames and demands exactly three — both defects it names (the cue on the sector's VALUE, and the un-consumed lap crossing) red it at four blips while every audibility case stays green. (2) `heathaze`'s wind was inaudible while sounding on 99.8% of frames and sweeping its band exactly as scored: it ran at **VOL 18 x ENVX 48 against the drone's 42 x 127 — about 7% of a music voice's amplitude.** Every case passed because every case asked WHETHER the voice sounded and none asked HOW LOUD; `ENVX > 0` is true at an amplitude nobody can hear, which is rule 2's failure mode wearing a costume — **reading the right register is not the same as asking the right question of it.** THE FIX IS ARCHITECTURAL, because volume alone would not have carried it: weather is now a NOISE CHANNEL IN THE SONG (`far_ridge_song` channel F) rather than a low-priority effect re-queued on a cadence. Four things follow and none is available to an effect — a music channel cannot be ducked for a cue or dropped for priority; `&` slurs send no key-off so it is continuous by construction with no cadence to keep in step with an effect length; it mixes in the score's own units; and **it reaches the echo**, which is the one that actually decided it, because the 128 ms buffer is what smears the LFSR's edges into a rush and the identical band played dry is tape hiss. Measured after: VOL 36 x ENVX 127, a ratio of 0.86 against the loudest music voice, held on 600/600 frames with the noise clock walking 16..23. The `wind` EFFECT was withdrawn rather than left unused, because leaving it leaves the trap — the next author wanting wind would reach for `SFX::wind` and land back on the 7% bed — and it was the last id in the last list, so removing it renumbered nothing where anywhere else would have obliged another whole-tree relink. **The constraint that makes this a rule and not a preference is the single noise generator**: a song channel holding it is muted whenever an effect plays noise, so the technique is available exactly to a rail whose cues are not noisy, which the composition must check (`heathaze` qualifies: its only cue is a pluck). The replacement case asserts a RATIO of `VOL x ENVX` against the song's own loudest voice rather than presence, and its bar was set by a plant rather than by eye — the first attempt, 0.25 chosen by guessing, PASSED a deliberately-too-quiet `v3` build at 0.29, and that same plant exposed a bug in the new helper itself, which was multiplying the SAMPLE NUMBER by the envelope because the fixture recorded no volume. **An audibility claim needs a too-quiet plant the way a cadence claim needs a per-frame one.** **AND A DENOMINATOR MOVED WHILE THIS WORK SAT (2026-09-19).** The audio branch was pushed, green under the landing gate, and never merged — no PR was opened — so `main` gained the `aurora` rail underneath it and the tree went from 41 rails to 42. The audio count is unchanged at 28: `aurora` is an end-credits sky with no game events and no `audio` claim, so it joins the silent set by the same argument the others use. The lesson is not about the count. **A branch that is green and unmerged is invisible**: a separate session built the homebrew for hardware testing straight off `main` and got none of this, which is how the gap was found. Nothing in the gate block can see it, because there is nothing wrong with the tree — only with where it is. |
...
**All 186 dirs under `engine/features/` accounted for.** 96 feature &middot; 49 blob &middot; 3 companion &middot; 28 consumer &middot; 10 game_logic.
...
|---|---|---|---|---|
| `audio` | **feature** | global | `dp`, `wram`&times;2, `reg`, `spc` | `tad_rom` |
...
| `aur_pres` | **feature** | unused | `dp`&times;2 | `aur_hue`, `aur_write` |
...
| `aur_bg` | **THE TWO LAYERS OF THE TREE'S FIRST MODE-3 SCENE**, and the depths are the whole reason the mode is what it is. `MODE_BPP[3]` is bg1 8bpp + bg2 4bpp, so the CHR claims are 64 and 32 bytes a tile and O9 joins each to the mode. **BG1 CLAIMS NO PALETTE AT ALL** — under `aur_mode`'s `direct_color` a direct-colour layer consults no CGRAM word (SnesPpu.cpp:1071), so the only `cgram` claim here is BG2's sixteen, and those sixteen are exactly used: transparent, far ridge, near ridge, cliff, rim, two star levels, and a nine-step anti-aliased ink ramp. It also owns the four scroll ports and writes them once, to zero — claimed **because the rail deliberately does not scroll**, not because it does: the ordered dither that makes an 8bpp gradient readable is destroyed by sliding neighbouring scanlines across it, so the ports are owned so that nothing else can move them |
| `aur_mode` | **MODE 3 AND `direct_color`, AND THE MODE IS FORCED RATHER THAN CHOSEN.** Direct colour acts on an 8bpp layer and nothing else (`GetRgbColor`'s arm is guarded by `bpp == 8 && directColorMode` alone, :1071), so the layer that wants it must be the 8bpp one — and the question becomes which mode pairs one with a companion able to hold everything that must NOT be direct. `MODE_BPP` answers it: mode 7 has no second layer, mode 4's bg2 is 2bpp (four colours for two ridges, a cliff, two star levels and a nine-step ink ramp, which it cannot), and mode 3 is bg1 8bpp + bg2 4bpp. `mil_mode_dc` set the same bit in mode 4; what this adds is the first composition where the SECOND layer had a job the mode had to be picked for |
| `aur_obj` | **THREE FIGURES, AND THE REASON THEY ARE SPRITES IS CGRAM AND NOT MOTION** — they never move. BG2's palette is exactly full at sixteen, and sprites read CGRAM at `128 + (palette << 4) + colour` (:960), so on OBJ the figures get their own sixteen and can carry a cold rim down the edge the curtains light instead of being flat silhouettes sharing the cliff's one entry. 16x32 sprites (OBSEL size pair 6), which the PPU reads as tiles N, N+1, N+16, N+17, N+32, N+33, N+48, N+49 off its 16-TILE-WIDE OBJ grid — the second row of a tall sprite is +16 tile numbers away, not +2 — so the page is four rows of that grid and the three interleave into it at N = 0, 2 and 4. OBJ is 4bpp in every mode: mode 3's 8bpp belongs to bg1 alone (:770 fetches sprite pixels through `GetTilePixelColor<4>` with the depth in the template argument) |
| `aur_hue` | **ON A DIRECT-COLOUR LAYER, COLOUR ANIMATION IS CHR TRAFFIC — and the price is the point.** A colour cycle is the classic INDEXED trick: rewrite one CGRAM word and every pixel using it changes at once, for two bytes. Direct colour is exactly the mode that gives that up — the pixel IS the colour, so there is no palette to cycle and the colour lives in the tiles. This rail therefore carries **sixteen copies of every tile the aurora tints, 311,296 B of art**, for an effect an indexed layer buys for two bytes a frame. That number is the honest counterweight to `aur_mode`'s zero-CGRAM headline, and both are the same fact seen from opposite ends. **THE PALETTE FIELD CANNOT DO IT**, which is why the rail's first cut was wrong rather than merely cheaper: the field supplies one LOW BIT per channel (2 of 31 in red and green, 4 in blue — the pixel gives blue only two bits, so its field bit is twice as loud as the others), which cannot carry a teal curtain to violet; and being per TILE, anything driven by it moves in 8x8 blocks, which read as pixelation rather than as light. The tinted tiles are given a CONTIGUOUS run of BG1 indices so a slice is one VMADD and one transfer, and that run is assigned in a **SCATTERED** order so a frame's share is spread over the whole sky rather than sweeping down it. The effect rests on the resulting two-phase mix being INVISIBLE, which is measured rather than assumed: adjacent phases are about five degrees of hue apart, under the dither's own noise, and on the shipped ROM **four 8x8 cells of the sky change in a hundred and twenty frames** — the curtains drift with nothing ever appearing to move. **A screen-coherent order was built, measured and rejected**, and the record is kept because the measurement was right and the conclusion was not: ordered bottom-up the phase boundary climbs, and it does NOT show as a colour step (across six cursor positions the largest step between adjacent tile rows sits at the hills' edge and never moves with the cursor). It was dropped anyway — a coherent front is legible as MOTION even where it is not legible as a seam, and the drift read as a wipe. The order it was introduced for, an aurora that RISES out of an unlit base page, went with it; docs/100 §14.5 records both halves. **The blob is `bank_tiled`, and the invariant that matters is not the obvious one**: a window holds 512 WHOLE 8bpp tiles, so a boundary never splits a TILE — but a slice is 50 tiles and 7 of the cycle's 72 slices cross a boundary anyway, and A1B is CONSTANT, so an uncut transfer wraps inside its own bank to $0000 and reads the WRAM mirror. Mesen's uninitialised-read detector named that in one run where a screenshot could not have: the picture was still recognisably an aurora, with a few tiles of garbage in it. A crossing slice is armed as two transfers, and the destination needs no second VMADD because the port auto-increments |
| `aur_rom` | `aur_bg` / `aur_obj` / `aur_hue` / `aur_write` — the credits scene's nine blobs, every one padded by the generator to the size claimed so the claim, the packer's order and `main.asm`'s `.assert`s are one number. Two are worth naming: `aur_hue` is **327,680 B** of precomputed phases and is the one claim in this tree that a 32 KB LoROM window cannot hold, so it is `bank_tiled` — clean here only because 32,768/64 is 512 WHOLE 8bpp tiles, so a chunk boundary never splits a tile (it does split transfers, which is a separate defect this rail paid for; see `aur_hue`); `aur_write` is the pen as a delta stream rather than a second copy of the finished word |
| `aur_write` | **THE WRITING, AS A CHR DELTA STREAM, AND THE SHAPE IS FORCED BY THE PEN.** It crosses tiles DIAGONALLY, so at any moment a tile is partly inked: a tilemap reveal would climb the word in eight-pixel stairs, and generating CHR on the 65816 per frame is out of the question. So the tiles that change in each frame are computed at build time — 168 uploads over 70 frames, never more than five in one, which is five 32-byte transfers in the worst VBlank. Start replays it by re-uploading the pen's own tile run, which is the LAST contiguous slice of the bg2 CHR page and is emitted **holding the ground under the word rather than blank**: an empty BG2 tile is transparent, and BG1's sky would otherwise show through the black band in exactly the rectangles the pen is about to fill (which is what the first cut did) |
| `aur_pres` | **THE RAIL IS SOMETHING YOU WATCH, AND A THING YOU WATCH HAS TO END.** Every other rail in this tree is something you DO; this one is a card, and left to itself it has no shape — the pen spends itself in seventy frames, the aurora finishes rising in one pass, and nothing after that marks time. So the beats are a feature: black, a bare sky brightening, the pen and the rise together, the finished card held, black again, and round. **THE BEATS WAIT ON DIFFERENT THINGS**, deliberately — the two ramps wait on `fade` going idle rather than on a frame count, so retuning the ramp cannot desynchronise them; PLAY watches the PEN's own frame counter, so it ends when the word is written rather than on a count that would have to be retuned beside it; the reset waits on the ink draining. Only the held card is a tuned number, and it is the beat where the colour is the only thing happening. **THE LOOP PUTS BACK THE INK AND NOTHING ELSE, and that is the interesting part**: the hue cursor runs underneath every beat and no beat touches it, so each pass opens on whatever hue the drift has reached. Measured on the ROM: the loop closes at 390 frames and successive passes stand at phases 0, 1, 2 — sixteen laps to travel the rail's whole 51-second journey from cyan-teal to violet. An earlier cut restored the CHR page at each lap; every pass then opened on the same teal and fifteen of the sixteen phases were unreachable, and every other case in the test module passed against it. The cost of carrying the colour is that passes are not pixel-identical, so it loops in SHAPE and not in pixels — which the clip does not mind, because the loop point is black. **It claims no register at all**: brightness has one writer in this tree (`sm_nmi_core` commits the shadow `fade` ramps), so a beat that wants black arms a direction and waits |
