...
| `aur_hue` | **feature** | unused | `dp`&times;9, `dma_init`, `dma` | `aur_rom`, `aur_bg` |
...
| `aur_pres` | **THE RAIL IS SOMETHING YOU WATCH, AND A THING YOU WATCH HAS TO END.** Every other rail in this tree is something you DO; this one is a card, and left to itself it has no shape — the pen spends itself in seventy frames, the aurora finishes rising in one pass, and nothing after that marks time. So the beats are a feature: black, a bare sky brightening, the pen and the rise together, the finished card held, black again, and round. **THE BEATS WAIT ON DIFFERENT THINGS**, deliberately — the two ramps wait on `fade` going idle rather than on a frame count, so retuning the ramp cannot desynchronise them; the rise waits `AUR_RATE_LEN` ticks, which is EXACTLY one pass because the generated rate curve sums to one phase over that many entries and the cycle reads one entry a tick, so the beat ends when the aurora has finished without watching a cursor for a wrap; the reset waits on both restores draining. Only the held card is a tuned number. **THE LOOP IS NOT A RESTART, and that is the interesting part**: only the ink and the aurora's CHR page are put back, and the HUE CURSOR IS LEFT RUNNING, so each pass rises in the colour the cycle has reached rather than the one before it. Measured on the ROM: the loop closes at 398 frames and successive passes start at phases 0, 2, 4, 6 — eight loops to travel the rail's whole 51-second journey from cyan-teal to violet. A loop that reset the phase would show the first two seconds of that forever. The cost is that passes are not pixel-identical, so it loops in SHAPE and not in pixels — the honest trade, and the right way round. **It claims no register at all**: brightness has one writer in this tree (`sm_nmi_core` commits the shadow `fade` ramps), so a beat that wants black arms a direction and waits |
