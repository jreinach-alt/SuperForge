...
**All 174 dirs under `engine/features/` accounted for.** 87 feature &middot; 48 blob &middot; 3 companion &middot; 26 consumer &middot; 10 game_logic.
...
| `mil_opt` | **feature** | unused | `vram`, `dp`&times;4, `hdma`, `reg`, `dma` | `mil_rom` |
| `mil_rom` | **blob** | unused | `rom`&times;6 | &mdash; |
...
| `mil_rom` | `mil_bg` + `mil_opt` — six blobs from `tools/gen_mill_assets.py`, all procedural: the five-tile 8bpp BG1 set (320 B — 64 bytes a tile, which is what 8bpp costs and why the budget that bought smelter fifteen tiles buys about sixty here), the five-tile 2bpp BG2 set (80 B), two 2,048-byte 32x32 tilemaps, ONE 136-byte palette blob carrying both CGRAM groups at `SMIL_PAL2_OFF` (one blob and not two for smelter's reason: the uploader reaches the second group by an offset from the first base symbol, and two claims are two blobs the packer orders by SIZE — a signed distance whose sign is not this file's to decide), and the 4,160-byte row table: 64 complete mode-4 offset rows plus a flat CONTROL at a 64 B stride. The control row is a complete row with every value at rest AND EVERY ENABLE BIT AND EVERY AXIS BIT STILL SET, so the flat picture and the moving one differ in the table and in nothing else |
