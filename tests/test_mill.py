...
# --------------------------------------------------------------------------
# THE MELT: the table read in BANDS — three rows, one BG3VOFS write a band
# --------------------------------------------------------------------------
...
# first line (`[[claims.offset_bands]]`, docs/100 §13.7), and the PPU fetches
# whatever row that port names. Every case below reads the PICTURE for it —
# the rows that move, the axis a column moves on, the rows that hold still —
# and joins it to the words the ROM holds in each table row.
SCENE_MELT = 2
JOY_DOWN = {"down": True}
MELT_CAM = _art("SMIL_MELT_CAM")
BAND_A = _art("SMIL_BAND_A_LINES")
BAND_B = _art("SMIL_BAND_B_LINES")
BAND_C = _art("SMIL_BAND_C_LINES")
RIPPLE_AMP = _art("SMIL_RIPPLE_AMP")
TAB_WORDS = _vsym("ES_V_MIL_TAB")


def _melt(key):
    for line in (BUILD / "mil" / "engine_state_melt.inc").read_text().splitlines():
        head, _, rest = line.partition("=")
        if head.strip() == key:
            return int(rest.split(";")[0].strip().replace("$", "0x"), 0)
    raise KeyError(f"{key} is not in engine_state_melt.inc")


def _wram(name, scene="melt"):
    """A WRAM placement's offset, global or this scene's."""
    for p in [*MAP["globals"], *MAP["scenes"][scene]["placements"]]:
        if p["sym"] == name and p["class"] == "wram":
            return p["start"]
    raise KeyError(name)


def to_melt(m):
    """The boot lobby, the hall, then DOWN on the parked car: the melt."""
    to_hall(m)
    for _ in range(400):
        if m.read_bytes(W, DP_SM + 2, 1)[0] == 0:
            break
        ...
        pytest.fail("the hall never finished arriving")
    m.advance(3, pad1=JOY_DOWN)
    for _ in range(600):
        if scene(m) == SCENE_MELT:
            break
        m.advance(1)
    else:
        pytest.fail("DOWN on the car never reached the melt")
    for _ in range(400):
        if m.read_bytes(W, DP_SM + 2, 1)[0] == 0:
            break
        m.advance(1)
    else:
        pytest.fail("the melt never finished arriving")
    first = m.read_u16(W, DP_SHOWN)
    for _ in range(600):
        if m.read_u16(W, DP_SHOWN) != first:
            return
        m.advance(1)
    pytest.fail("the offset table never advanced in the melt")


def table_row(m, r):
    """Table row `r` as the PPU holds it, re-indexed by SCREEN column."""
    raw = m.read_bytes(MemoryType.SnesVideoRam, (TAB_WORDS + r * COLS) * 2, COLS * 2)
    words = [raw[i] | (raw[i + 1] << 8) for i in range(0, COLS * 2, 2)]
    return [None if sc < LEAD else words[sc - LEAD] for sc in range(COLS)]


def until_both_rows_move(m, cap=120):
    ...
    for _ in range(cap):
        ...
        if table_row(m, 0) != r0 and table_row(m, 1) != r1:
            return
    pytest.fail("the hall's row and the ripple row did not both advance")
...
def test_the_melt_reads_three_rows_and_the_deck_band_stands_still(tmp_path):
    ...
    between them has — the same 32 columns, three rows of the table, and
    which row a line reads is decided by the band it is in. The channel's
    first moving line is the band's declared first line: the BG3VOFS write
    lands where the table says it does."""
    with Machine(str(ROM)) as m:
        to_melt(m)
        assert m.read_u16(W, DP_CAM) == MELT_CAM
        a = shot(m, tmp_path / "a.png")
        until_both_rows_move(m)
        b = shot(m, tmp_path / "b.png")
    moved = changed_rows(a, b)
    deck = [y for y in moved if BAND_A <= y < BAND_A + BAND_B]
    assert not deck, f"the deck band moved on rows {deck}"
    assert [y for y in moved if y < BAND_A], "the machine band did not move"
    below = [y for y in moved if y >= BAND_A + BAND_B]
    assert below, "the channel band did not move"
    assert below[0] == BAND_A + BAND_B, (
...
def test_a_column_carries_an_h_word_in_one_band_and_a_v_word_in_another(tmp_path):
    """A belt column: its row-0 word drives BG2 horizontally and its row-1
    word drives BG1 vertically. In the belt band the tread slides SIDEWAYS
    between two frames; in the channel band the SAME screen column's art
    slides UP OR DOWN by exactly the difference of its two ripple words.
    Mode 4 makes the axis a per-column choice; bands make it per-column,
    per-band, in one frame."""
    with Machine(str(ROM)) as m:
        to_melt(m)
        r0, r1 = table_row(m, 0), table_row(m, 1)
        ...
        a = shot(m, tmp_path / "a.png")
        ka = {sc: (MELT_CAM - (r1[sc] & V_MASK)) & V_MASK for sc in cols}
        ha = {sc: r0[sc] & H_MASK for sc in cols}
        for _ in range(120):
            m.advance(1)
            r1b, r0b = table_row(m, 1), table_row(m, 0)
            if any(((MELT_CAM - (r1b[sc] & V_MASK)) & V_MASK) != ka[sc] for sc in cols) \
                    and any((r0b[sc] & H_MASK) != ha[sc] for sc in cols):
                break
        else:
            pytest.fail("neither the belt phase nor the ripple changed")
        b = shot(m, tmp_path / "b.png")
        kb = {sc: (MELT_CAM - (r1b[sc] & V_MASK)) & V_MASK for sc in cols}
    y0, y1 = BELT_ROW * 8 - MELT_CAM, BELT_ROW * 8 - MELT_CAM + 16
    assert 0 <= y0 and y1 <= BAND_A, "the belt band must lie inside band A"
    c0, c1 = BAND_A + BAND_B, 224
    # THE HORIZONTAL HALF, THE HALL'S WAY: a belt column's tread has changed
    # between the frames and has NOT slid vertically. Whether it visibly
    # moved sideways as one 8-px strip cannot be asked of a column (the hall's
    # axis case says why: the housing over the belt is BG1 and the tread is
    # locally one colour under it), so the claim is the axis, asserted on
    # every column, and liveness in aggregate.
    ran = 0
    for sc in cols:
...
def test_y_holds_both_moving_bands_still(tmp_path):
    ...
    reads the hall's control row and the channel band the ripple's, and the
    ...
    assert all(w is None or (w & V_MASK) in (0, MELT_CAM) for w in r0 + r1), \
...
def test_the_band_channel_is_the_composition_s_own_and_is_armed(tmp_path):
    """The declaration and the machine agree: the scene's map names the
    synthesized channel, the HDMA table the ROM built holds the three declared
    band lengths and row selectors, and the HDMAEN shadow the NMI commits has
    that channel's bit in the melt and NOT in the hall."""
    vo = MAP["scenes"]["melt"]["video_offset"]
    assert vo["bands"] == 3 and vo["rowsel"] == "mil_bands_rowsel"
    ch = _melt("ES_H_MIL_BANDS_ROWSEL_CH")
    chans = [c for c in MAP["scenes"]["melt"]["channels"] if c["name"] == "mil_bands_rowsel"]
    assert chans and chans[0]["registers"] == ["BG3VOFS"] and chans[0]["ch"] == ch
    assert "video_offset" not in MAP["scenes"]["hall"] or \
        MAP["scenes"]["hall"]["video_offset"]["bands"] == 1
    row_vofs = _melt("ES_OPT_MELT_ROW_VOFS")
    nmi = _dp("ES_SM_NMI")
    with Machine(str(ROM)) as m:
        to_hall(m)
        assert not m.read_bytes(W, nmi + 2, 1)[0] & (1 << ch), "armed in the hall"
        m.advance(3, pad1=JOY_DOWN)
        for _ in range(600):
            if scene(m) == SCENE_MELT:
                break
            m.advance(1)
        m.advance(4)
        assert m.read_bytes(W, nmi + 2, 1)[0] & (1 << ch), "not armed in the melt"
        tab = m.read_bytes(W, _wram("ES_MIL_BANDTAB"), 10)
    assert list(tab) == [BAND_A, 0, 0,
                         BAND_B, 2 * row_vofs, 0,
                         BAND_C, 1 * row_vofs, 0,
                         0], list(tab)
    assert BAND_A + BAND_B + BAND_C == 224


def test_up_in_the_melt_brings_him_back_standing_on_the_car(tmp_path):
    with Machine(str(ROM)) as m:
        to_melt(m)
        left_at = m.read_u16(W, DP_PX)      # the ride X: where the car is cut
        m.advance(3, pad1=JOY_UP)
        for _ in range(600):
            if scene(m) == SCENE_HALL:
                break
            ...
            pytest.fail("UP in the melt never reached the hall")
        for _ in range(400):
            if m.read_bytes(W, DP_SM + 2, 1)[0] == 0:
                break
            m.advance(1)
        m.advance(2)
        assert m.read_u16(W, DP_BOARD) == 0 and m.read_u16(W, DP_CAR) == 0
        assert m.read_u16(W, DP_PX) == left_at
        x, y = rider_xy(m)
    assert y == STAND_Y - RIDER_BOX - CAM_MAX, "he is not standing on the car"
