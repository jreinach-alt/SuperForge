...
# =============================================================================
# THE THREE FIGURES ARE TRACED FROM THE OWNER'S REFERENCE RENDER
# =============================================================================
# DO NOT REDRAW THESE BY HAND, and do not replace them with a formula. Both
# were tried and both failed, in ways worth recording so the next person does
# not spend the afternoon again:
#
#   * THE FORMULA. A cone whose half-width tapered from the feet up, with a
#     CIRCLE ON TOP for a head joined by a one-pixel neck. Ball, neck, and the
#     widest point at the FEET — three of the three things that say "board
#     game token", and that is exactly how it read.
#   * HAND-AUTHORING. Several rounds of ASCII silhouettes with legs, hoods and
#     folded arms. Better than the formula and still not the reference: the
#     shapes were invented to match a description instead of copied from the
#     picture, and every round drifted somewhere new.
#
# What is below is the reference render's own three silhouettes, lifted from
# it. The method, so it can be repeated if the reference is ever re-rendered:
# threshold at luminance 26, take connected components ABOVE the ground band
# (which starts at y=380 and would otherwise weld all three into one), and
# downsample each mask by AREA COVERAGE at >= 50% — not by nearest-neighbour,
# which on a dithered source destroys the outline it is supposed to preserve.
# One scale factor for all three, set so the tallest fills the 32-row sprite,
# which is what keeps their relative heights the reference's.
#
# Measured on the reference at 2x: 22x60, 30x68 and 22x56 -> 10x28, 14x32 and
# 10x26 here. They are HOODED AND CLOAKED — a pointed crown continuing
# straight into a body that flares to the ground, with no separate head and no
# neck. The middle one is taller, broader, and its hood is flat-topped rather
# than pointed.
FIG_LEFT = """
........#.......
.......###......
...
# (centre x on screen, the map). The heights fall out of the maps — 28, 32, 26
# — so a redraw cannot disagree with a constant.
...
def figure_px(n):
    ...
    THE REFERENCE DRAWS THEM FLAT BLACK and takes its contrast from the
    background — measured on it, the hill behind runs 18-40 in luminance
    against a figure interior of 17. That is not available here: the cliff
    behind them is 8 and the interior is 0, so a rim stands in for it, graded
    down the body because the curtains are above.

    It runs the whole way round rather than down the left only. An earlier cut
    lit each row's left edge alone, which made the left contour the entire
    figure and hid the hood, the shoulders and the flare of the cloak.
    """
    rows = fig_rows(n)
    h = len(rows)
    ...
    for r in range(h):
        t = 1.0 - r / float(h - 1)             # 1 at the crown, 0 at the hem
        hi = 3 if t > 0.40 else 2
        for c in range(OBJ_W):
            if not solid(r, c):
                continue
            if not solid(r, c - 1) or not solid(r - 1, c):
                v = hi
            elif not solid(r, c + 1):
                v = max(2, hi - 1)
            else:
                v = 1
            out[OBJ_H - h + r][c] = v
