#!/usr/bin/env python3
"""gen_cf_assets.py — deterministic art + palettes for the `camera_follow` rail.

Emits (byte-identical on re-run, pure integer math):

  cf_bg_chr.bin    2 x 4bpp BG tiles, 32 B each = 64 B
                   tile 0 EMPTY (all indices 0) · tile 1 SOLID index 1
  cf_bg_pal.bin    16 BGR555 words = 32 B  (BG palette group 0, CGRAM 0..15;
                   word 0 IS the backdrop slot — see cf_bg/feature.toml)
  cf_obj_chr.bin   1 x 4bpp OBJ tile = 32 B   solid index 1
  cf_obj_pal.bin   16 BGR555 words = 32 B  (OBJ palette 0, CGRAM 128..143)
...
2 and 3) — a 4bpp tile whose every pixel is colour index 1. The
checkerboard's other cell is tile 0, all zeros: index 0, which a 4bpp BG
renders as the backdrop.

So the picture is *green squares on the backdrop*, and the two "colours" of
the checkerboard are one palette entry and one transparency. Tile 0 is written
EXPLICITLY rather than inherited from a cleared VRAM (rule 5 — every byte the
feature reads is a byte the feature wrote).

Colours:
  OBJ_RED  = $001F   player red
  BG_GREEN = $03E0   checkerboard green

(These are byte-identical to the scroller rail's blobs — the two rails share
the one solid tile and the same two colour constants — but each rail's blob
is generated by its own claim-named generator so the register's per-rail
supply stays honest and neither rail can silently re-point the other's art.)

NO SILENT MASKING (CLAUDE.md's asset-encoder rule): `encode_4bpp` asserts
every pixel index is 0..15 and every palette entry is a 15-bit BGR value. An
...
# is why cf_bg claims it rather than composing `backdrop`. It is BLACK, and it
# is written HERE rather than left to `sf_coldstart`'s CGRAM clear — the
...
def main(outdir):
    ...
    files = {
        "cf_bg_chr.bin": encode_4bpp(EMPTY_TILE, "bg tile 0 (empty)")
                         + encode_4bpp(SOLID_TILE, "bg tile 1 (solid)"),
        "cf_bg_pal.bin": encode_pal(BG_PAL, "bg palette"),
        "cf_obj_chr.bin": encode_4bpp(SOLID_TILE, "obj tile 0 (solid)"),
        "cf_obj_pal.bin": encode_pal(OBJ_PAL, "obj palette"),
    }
