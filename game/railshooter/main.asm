...
;
; EVERY NUMBER HERE MOVED WHEN `audio` WAS COMPOSED. `tad_export`'s 16,384 B
; half-window takes the top of window 1, and the small blobs — which used to
; have a window of their own — now fit BESIDE it in what is left of window 1,
; pushing the pose chunks to windows 2..5 and the Mode 7 plane to 6. The
; offsets in the two `.sprintf`s below are the only place that arithmetic is
; written down, and ld65 refused the build BY NAME (`poses_ab chunk bank
; drifted`) until they matched. Same shape as game/microzero/main.asm.
.repeat ES_R_POSES_AB_CHUNKS, PI
.segment .sprintf("BANK%d", PI + 2)
