...
MAIN:
    ...
    ; STEREO, because the song it is about to load is PANNED — the mid pulse
    ; sits left and the arpeggio right, and in MONO (TAD's default,
    ; tad-audio.inc:123) the driver collapses every channel to centre. The
    ; mode only takes effect at the next song load (tad-audio.inc:525), so it
    ; is set HERE — after Tad_Init, which initialises it, and before
    ; Tad_LoadSong.
    lda #TadAudioMode::STEREO
    sta Tad_audioMode
    ; The ACTION rails' song — kit, sixteenth bass, saw lead, scored on A-F so
    ; nothing of it sits on the two channels a sound effect ducks.
