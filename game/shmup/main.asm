; =============================================================================
; shmup — the vertical-scrolling shooter. ROM skeleton.
; =============================================================================
; NO RESOURCE IS WRITTEN DOWN HERE. Every RAM/VRAM/CGRAM address, channel and
...
.include "shmup.inc"                ; the rail's geometry + state vocabulary
.include "header.inc"
...
; build/sh/allocation_report.txt. Each site .asserts its blob's linker
...
.segment "BANK2"
shm_bg_chr_bin:
    .incbin "shm_bg_chr.bin"
.assert ^shm_bg_chr_bin = ES_R_SHM_BG_CHR_ROM_BANK, error, "shm_bg_chr bank drifted from allocator claim"
.assert .loword(shm_bg_chr_bin) = ES_R_SHM_BG_CHR_ROM_ADDR, error, "shm_bg_chr addr drifted from allocator claim"
font_bin:
...
shm_obj_chr_bin:
    .incbin "shm_obj_chr.bin"
.assert ^shm_obj_chr_bin = ES_R_SHM_OBJ_CHR_ROM_BANK, error, "shm_obj_chr bank drifted from allocator claim"
.assert .loword(shm_obj_chr_bin) = ES_R_SHM_OBJ_CHR_ROM_ADDR, error, "shm_obj_chr addr drifted from allocator claim"
shm_bg_pal_bin:
    .incbin "shm_bg_pal.bin"
.assert ^shm_bg_pal_bin = ES_R_SHM_BG_PAL_ROM_BANK, error, "shm_bg_pal bank drifted from allocator claim"
.assert .loword(shm_bg_pal_bin) = ES_R_SHM_BG_PAL_ROM_ADDR, error, "shm_bg_pal addr drifted from allocator claim"
shm_burst_pal_bin:
    .incbin "shm_burst_pal.bin"
.assert ^shm_burst_pal_bin = ES_R_SHM_BURST_PAL_ROM_BANK, error, "shm_burst_pal bank drifted from allocator claim"
.assert .loword(shm_burst_pal_bin) = ES_R_SHM_BURST_PAL_ROM_ADDR, error, "shm_burst_pal addr drifted from allocator claim"
shm_foe_pal_bin:
    .incbin "shm_foe_pal.bin"
.assert ^shm_foe_pal_bin = ES_R_SHM_FOE_PAL_ROM_BANK, error, "shm_foe_pal bank drifted from allocator claim"
.assert .loword(shm_foe_pal_bin) = ES_R_SHM_FOE_PAL_ROM_ADDR, error, "shm_foe_pal addr drifted from allocator claim"
shm_ship_pal_bin:
    .incbin "shm_ship_pal.bin"
.assert ^shm_ship_pal_bin = ES_R_SHM_SHIP_PAL_ROM_BANK, error, "shm_ship_pal bank drifted from allocator claim"
.assert .loword(shm_ship_pal_bin) = ES_R_SHM_SHIP_PAL_ROM_ADDR, error, "shm_ship_pal addr drifted from allocator claim"
.segment "CODE"
...
; cannot reach them). The scroll commit is the exception to "call it
; unconditionally": BG1VOFS belongs to the play scene alone, and the title —
; which runs BG3 by itself and owns no BG1 register — must not have one
; written under it. scene_mgr sets `cur = next` BEFORE calling enter and holds
; NMI masked across the whole switch (`@switch`, exit through enter), so the first NMI
; that sees SCENE_PLAY is the first one after play::enter armed the shadow.
sm_nmi_hook:
    .a8
    .i16
