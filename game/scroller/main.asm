...
; =============================================================================
; A green checkerboard fills BG1, the d-pad scrolls it in all four directions
; at 2 px/frame, and a red sprite holds screen centre so the world appears to
; slide beneath it — the smallest end-to-end tour of the BG pipeline plus
; sprite-over-BG compositing this kit has.
;
; ONE SCENE, no edges. This rail's subject is one screen and one camera, so a
; title screen would be surface the subject does not have. scene_mgr is
; composed anyway because it owns the INIDISP commit and the frame sync every
; SuperForge rail's main loop is built on; `world` is simply its only
; destination.
...
; this rail has exactly one scene, and both scroller_bg and scroller_obj
; resolve its symbols from file scope, so a wrapper here would only hide them
; from the features that need them.
.include "engine_state_world.inc"   ; GENERATED — the world scene's map
.include "scroller.inc"             ; the rail's geometry + tuning
.include "header.inc"
...
.segment "BANK1"
scr_bg_chr_bin:
    .incbin "scr_bg_chr.bin"
.assert ^scr_bg_chr_bin = ES_R_SCR_BG_CHR_BANK, error, "scr_bg_chr bank drifted from allocator claim"
.assert .loword(scr_bg_chr_bin) = ES_R_SCR_BG_CHR_ADDR, error, "scr_bg_chr addr drifted from allocator claim"
scr_bg_pal_bin:
    .incbin "scr_bg_pal.bin"
.assert ^scr_bg_pal_bin = ES_R_SCR_BG_PAL_BANK, error, "scr_bg_pal bank drifted from allocator claim"
.assert .loword(scr_bg_pal_bin) = ES_R_SCR_BG_PAL_ADDR, error, "scr_bg_pal addr drifted from allocator claim"
scr_obj_chr_bin:
    .incbin "scr_obj_chr.bin"
.assert ^scr_obj_chr_bin = ES_R_SCR_OBJ_CHR_BANK, error, "scr_obj_chr bank drifted from allocator claim"
.assert .loword(scr_obj_chr_bin) = ES_R_SCR_OBJ_CHR_ADDR, error, "scr_obj_chr addr drifted from allocator claim"
scr_obj_pal_bin:
    .incbin "scr_obj_pal.bin"
.assert ^scr_obj_pal_bin = ES_R_SCR_OBJ_PAL_BANK, error, "scr_obj_pal bank drifted from allocator claim"
.assert .loword(scr_obj_pal_bin) = ES_R_SCR_OBJ_PAL_ADDR, error, "scr_obj_pal addr drifted from allocator claim"
.segment "CODE"

; --- the scene-scoped features (after the blobs their uploads read) --------
.include "scroller_bg.asm"
.include "scroller_obj.asm"
...
;
; Every writer programs its own VMADD where it needs one, so the order is free.
; oam_nmi_dma commits the sprite shadow the tick staged; scr_bg_nmi_commit
; publishes the camera to BG1HOFS/BG1VOFS.
sm_nmi_hook:
    .a8
    .i16
    jsr oam_nmi_dma                 ; commit the OAM shadow
    jsr scr_bg_nmi_commit           ; the camera -> BG1HOFS/BG1VOFS
    rts
...
MAIN:
    ...
    ; — the ramp never arms, INIDISP stays at brightness 0, and the ROM renders
    ...
    ; contract, which width-lint cannot see in either direction (CLAUDE.md rule
    ; 6, "the single-file limit remains"). It cost split_v_fight a real
