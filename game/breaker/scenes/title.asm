; =============================================================================
; title scene — "BRICK BUSTER" + PRESS START, and the round counter
; =============================================================================
; Deliberately plain (bg_text + backdrop, the two cheapest features in the
; tree). It has a job beyond decoration: it is the scene the arena is NOT
; running in, so returning here proves breaker_bg's teardown. A colour-math
; config or an HDMA channel left armed by `play` would tint or stripe this
; screen through registers it never wrote.
;
; It is ALSO where a restart passes through. `play` cannot rebuild its 180
; bricks on a running frame, and scene_mgr refuses a self-transition, so
; GAME OVER -> Start comes here and Start goes back — which re-enters
; play::enter under forced blank, where rebuilding the wall is legal.
.scope title
...
enter:
    ...
    rep #$20
    .a16
    ldx #(ES_V_TEXT_MAP + 8*32 + 10)    ; row 8, col 10 (12 chars centred)
    jsr text_puts
    lda #.loword(s_press)
    sta z:ES_TXT_PTR
    ...
    jsr text_puts
    lda f:US_ROUNDS_LONG                ; the surviving-global-state render
...
.segment "RODATA"
s_logo:   .byte "BRICK BUSTER", 0
s_press:  .byte "PRESS START", 0
s_rounds: .byte "ROUNDS ", 0
.segment "CODE"
.endscope
