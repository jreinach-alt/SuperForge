...
| 5 | **Game works on NTSC and PAL** | **met, and past the literal reading** | Two layers. *It runs on both:* the 37 games in the `docs/93` sweep boot, run and render under PAL with nothing crashing, hanging or corrupting, and no per-scanline table under-covering the taller frame. *It runs at the same speed on both:* **32 of the 39** compose `region` + `tick_scale` — every playable game, plus the two screen-effect rails — and **all 32 are measured**, holding a real-time parity band of **0.994–1.027** against the **0.832** an uncompensated game reads ([`docs/98`](docs/98_region_fleet_landing.md) §1). The two rails added after that sweep were measured on 2026-08-28 at 0.99989 (`heathaze`) and 1.00127 (`lakeside`), inside the band rather than widening it. The mechanism and the header corrections it rides on are [`docs/97`](docs/97_region_r0_landing.md); the requirement it answers is [`docs/94`](docs/94_region_support_spec.md) §2. The seven that do not compose it are all determinism trials whose frame-indexed sweeps are the thing under test — each states its reason in its own `game.toml` (`docs/98` §1, §4). |
| 6 | **Works on real hardware** | **RUN ON A CART, 2026-09-19 — qualitative, one console, and NOT the whole tree** | **Somebody ran a cart.** The owner flashed the demos to an FxPak Pro and played them on their own SNES; all of them ran, and nothing crashed, hung or rendered wrong to the eye. That retires the flat "nothing here has ever run on hardware" this row carried for the project's whole life, and it is the single most load-bearing fact in this table, because every other row rests on Mesen2 agreeing with silicon and now something outside Mesen2 has agreed with it. **What it does NOT establish, stated so the row is not read past its evidence:** (a) it is an OWNER'S PLAY-TEST, not an instrumented measurement — it says the rails run and look right, not that any cycle count, parity figure or frame budget in these docs holds on silicon, and every number elsewhere is still Mesen2's; (b) the console's REGION was not recorded, so if it is NTSC then `docs/93` §12's PAL questions — what a PAL set actually shows, whether a real PAL SNES matches Mesen's 312-line model — are all still open, and if it is PAL then §12.1 is partly answered and worth writing down properly; (c) the images tested were built from `main`, so they carry NO audio work from `claude/procedural-sfx-rails-6hqalx` — the songs, the cues and the forked TAD driver have still never run on hardware, and the S-DSP is exactly where an emulator and a console are most likely to differ; (d) one console is one console, and the sample says nothing about revision spread. The design remains hardware-shaped by discipline — power-on RAM random under test, PPU writes blank-window only, DMA inside bank boundaries — and that discipline is now corroborated rather than merely argued. |
