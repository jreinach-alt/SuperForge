; =============================================================================
; shp_floor.asm — the split_h_persp rail's Mode 7 plane: one DMA, five colours
; =============================================================================
...
; whole rail's motion is the CAMERAS'; the world under them never changes.
;
; The blob labels (`shp_map_bin`, `shp_pal_bin`) are the game's .incbin claim
; sites in main.asm — the feature names them, the game backs them, and `make
; rom-unbacked` proves the bytes exist.

; The enter-time GP-DMA register file, addressed through the channel the shp_up
; dma_init claim names — a declared resource, not a hard-coded 0.
SHP_REGS = $4300 + ES_D_SHP_UP_CH * 16
...
; Tools/gen_split_h_persp_assets.py emits the blob already in that layout. DMA
...
floor_arm:
    ...
    lda #^shp_map_bin
    sta a:SHP_REGS + 4              ; A1B = source bank
    lda #ES_D_SHP_UP_DMAP
    sta a:SHP_REGS + 0              ; DMAP: A->B, 2 regs (mode 1) = the interleave
    lda #ES_D_SHP_UP_BBAD
    sta a:SHP_REGS + 1              ; BBAD: VMDATAL ($2118), so B+1 = $2119
    rep #$20
    .a16
    stz a:$2116                     ; VMADD = 0 (the Mode 7 base is fixed at 0)
    ldx #.loword(shp_map_bin)
    stx a:SHP_REGS + 2              ; A1T
    ldy #ES_R_SHP_MAP_SIZE
    sty a:SHP_REGS + 5              ; DAS (armed for THIS transfer)
    sep #$20
    .a8
    lda #(1 << ES_D_SHP_UP_CH)
    ...
    ;
    ; WORDS 1-2 CARRY NO RED AND WORDS 3-4 DO, and that is not decoration: it
    ; is this rail's per-band POSITION oracle. Camera A sits on the cool stripe
    ; and camera B on the warm one, so the red channel separates the two bands
    ; independently of their checker periods.
    sep #$20
    .a8
    lda #ES_C_SHP_PAL
...
:   lda f:shp_pal_bin, x
    ...
    ; — which is what lets both cameras' trapezoids recede forever without a
    ; clamp, and what makes the world's 1024-px period the only wrap the
    ; predictor has to honour. (BGMODE and TM are the scene's `scene_writes`;
    ; see this feature's feature.toml for the attribution.)
    sep #$20
    .a8
    stz a:$211A                     ; M7SEL
    rep #$20
