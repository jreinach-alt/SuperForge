# VENDORED: this file is carried in the tree as a frozen dependency rather than
# installed. Nothing imports it from outside; edit it here and re-run the suite.
"""
...
# CGRAM / screenshot). The mutation gate (G2, later work item) consumes the same
...
# --- Live hardware state: SnesState via GetConsoleState -----------------------
#
# The debugger's *peek* path cannot see the DMA controller: reads of $4300-$431F
# and $420C through MemoryType.SnesRegister (or SnesMemory) come back as zeros /
# open bus, because SnesDmaController exposes no Peek handler for them. That is
# a real limitation, independently reproduced (
# §3.7), and it is why the first pass of tests/test_decl_impl_channels.py
# asserted against the WRAM shadow instead of the registers — which let a
# genuinely undeclared HDMA channel armed in $420C pass a test named
# `test_no_undeclared_channel_is_enabled`.
#
# The DLL export that DOES reach it is GetConsoleState(BaseState&, ConsoleType)
# (InteropDLL/DebugApiWrapper.cpp), which memcpy-fills a whole `SnesState`
# (Core/SNES/SnesState.h). Three of its members are what we need:
#
#   SnesCpuState           Cpu  -> A, for the MDMAEN mask a firing site stores
#   SnesDmaControllerState Dma  -> DmaChannelConfig Channel[8] (DMAP/BBAD and
#                                  friends, decomposed into fields) plus
#                                  `uint8_t HdmaChannels` = the live $420C mask
#   InternalRegisterState  InternalRegs -> the live $4200 (NMITIMEN) bits
#
# LAYOUT PROVENANCE. `SnesState` is a plain C++ aggregate with no stable ABI, so
# the byte offsets below are *derived*, not guessed — reproduce them with
# tools/mesen_state_offsets.cpp (a short offsetof dump compiled against the
# Mesen2 headers; the file documents the exact command). Measured against
# Mesen2 as vendored by tools/setup.sh:
#
#   sizeof(SnesState)                 = 1440
#   offsetof(SnesState, Cpu)          =    8   (+ offsetof(SnesCpuState, A) = 16)
#   offsetof(SnesState, Dma)          = 1238
#   offsetof(SnesState, InternalRegs) = 1400   (== 1238 + 162, Dma's size)
#
# Hardcoding a derived offset is exactly the kind of thing that reads plausible
# bytes when it is wrong, so it is NOT trusted: every read runs
# _validate_snes_state_layout(), which rejects a buffer whose C++ `bool` bytes
# are not 0/1, whose TransferMode fields are not 0..7, or whose 9-bit H/V timers
# overflow. Tests additionally anchor the layout on values predicted
# independently of the DMA subsystem (see tests/test_decl_impl_channels.py::
# test_console_state_layout_anchor). The buffer handed to the DLL is
# deliberately far larger than the struct so a future Mesen2 that grows
# SnesState cannot write out of bounds — it would trip the validation instead.
_SNES_STATE_BUF_BYTES = 16384       # >> sizeof(SnesState) = 1440
_OFF_SNES_CPU_A = 16                # offsetof(SnesState, Cpu) + offsetof(A)
# SnesCpuState field offsets, RELATIVE to A (Core/SNES/SnesCpuTypes.h). Only A
# was mirrored originally; PC + K were added so a breakpoint hit can be
# attributed to the FIRING SITE and not merely to the channel encoding, which
# several claims share by construction.
_CPU_OFF_PC = 10                    # uint16 -> program counter
_CPU_OFF_K = 12                     # uint8  -> program bank
_CPU_OFF_EMULATION_MODE = 15        # bool   -> 6502 emulation mode
_CPU_OFF_IRQ_LOCK = 17              # bool
_CPU_OFF_NEED_NMI = 18              # bool
_CPU_OFF_STOP_STATE = 21            # SnesCpuStopState : uint8 (0/1/2)
_CPU_BOOL_OFFSETS = (_CPU_OFF_EMULATION_MODE, _CPU_OFF_IRQ_LOCK,
                     _CPU_OFF_NEED_NMI)
_OFF_SNES_DMA = 1238
_OFF_SNES_INTERNAL_REGS = 1400
_DMA_CH_STRIDE = 20                 # sizeof(DmaChannelConfig)
_DMA_OFF_HDMAEN = 160               # offsetof(SnesDmaControllerState, HdmaChannels)
_CONSOLE_TYPE_SNES = 0              # ConsoleType::Snes

# DmaChannelConfig field offsets (Core/SNES/DmaControllerTypes.h). The `bool`s
# are one byte each and MUST read back as exactly 0 or 1 — that is the
# structural check that makes a misaligned read loud instead of plausible.
_DCC_SRC_ADDRESS = 0                # uint16 -> $43x2/$43x3
_DCC_TRANSFER_SIZE = 2              # uint16 -> $43x5/$43x6 (counts down)
_DCC_HDMA_TABLE_ADDRESS = 4         # uint16 -> $43x8/$43x9
_DCC_SRC_BANK = 6                   # uint8  -> $43x4
_DCC_DEST_ADDRESS = 7               # uint8  -> $43x1 (BBAD)
_DCC_DMA_ACTIVE = 8                 # bool
_DCC_INVERT_DIRECTION = 9           # bool   -> DMAP bit 7
_DCC_DECREMENT = 10                 # bool   -> DMAP bit 4
_DCC_FIXED_TRANSFER = 11            # bool   -> DMAP bit 3
_DCC_HDMA_INDIRECT = 12             # bool   -> DMAP bit 6
_DCC_TRANSFER_MODE = 13             # uint8  -> DMAP bits 0-2
_DCC_HDMA_BANK = 14                 # uint8  -> $43x7
_DCC_HDMA_LINE_COUNTER = 15         # uint8  -> $43xA
_DCC_DO_TRANSFER = 16               # bool
_DCC_HDMA_FINISHED = 17             # bool
_DCC_UNUSED_CONTROL_FLAG = 18       # bool   -> DMAP bit 5
_DCC_UNUSED_REGISTER = 19           # uint8  -> $43xB/$43xF
_DCC_BOOL_OFFSETS = (
    _DCC_DMA_ACTIVE, _DCC_INVERT_DIRECTION, _DCC_DECREMENT,
    _DCC_FIXED_TRANSFER, _DCC_HDMA_INDIRECT, _DCC_DO_TRANSFER,
    _DCC_HDMA_FINISHED, _DCC_UNUSED_CONTROL_FLAG,
)

# InternalRegisterState field offsets (Core/SNES/InternalRegisterTypes.h).
_IR_AUTO_JOYPAD = 0                 # bool -> NMITIMEN bit 0
_IR_FAST_ROM = 1                    # bool -> MEMSEL ($420D) bit 0
_IR_ENABLE_NMI = 2                  # bool -> NMITIMEN bit 7
_IR_ENABLE_H_IRQ = 3                # bool -> NMITIMEN bit 4
_IR_ENABLE_V_IRQ = 4                # bool -> NMITIMEN bit 5
_IR_H_TIMER = 6                     # uint16
_IR_V_TIMER = 8                     # uint16
_IR_IO_PORT_OUTPUT = 10             # uint8
_IR_BOOL_OFFSETS = (_IR_AUTO_JOYPAD, _IR_FAST_ROM, _IR_ENABLE_NMI,
                    _IR_ENABLE_H_IRQ, _IR_ENABLE_V_IRQ)


class SnesStateLayoutError(RuntimeError):
    """GetConsoleState returned a buffer that does not match the mirrored layout.

    Raised instead of silently returning plausible-looking garbage. If this
    fires, the Mesen2 core in use has a different `SnesState` layout than the
    offsets above: re-derive them with tools/mesen_state_offsets.cpp.
    """


class SnesDmaChannel:
    """One DMA/HDMA channel's live hardware configuration ($43x0-$43xF).

    `dmap` and `bbad` are the two bytes a claim's declaration emits, rebuilt
    from Mesen's decomposed fields exactly the way Mesen's own $43x0 read
    handler does it (SnesDmaController.cpp:536-543), so comparing them against
    ES_H_<CLAIM>_DMAP / ES_H_<CLAIM>_BBAD is a like-for-like comparison.
    """

    __slots__ = ("ch", "dmap", "bbad", "src_address", "src_bank",
                 "transfer_size", "hdma_table_address", "hdma_bank",
                 "hdma_line_counter", "transfer_mode", "indirect", "fixed",
                 "decrement", "invert_direction", "unused_control_flag",
                 "dma_active", "do_transfer", "hdma_finished")

    def __init__(self, ch: int, raw: bytes):
        self.ch = ch
        self.transfer_mode = raw[_DCC_TRANSFER_MODE]
        self.indirect = bool(raw[_DCC_HDMA_INDIRECT])
        self.fixed = bool(raw[_DCC_FIXED_TRANSFER])
        self.decrement = bool(raw[_DCC_DECREMENT])
        self.invert_direction = bool(raw[_DCC_INVERT_DIRECTION])
        self.unused_control_flag = bool(raw[_DCC_UNUSED_CONTROL_FLAG])
        self.dma_active = bool(raw[_DCC_DMA_ACTIVE])
        self.do_transfer = bool(raw[_DCC_DO_TRANSFER])
        self.hdma_finished = bool(raw[_DCC_HDMA_FINISHED])
        self.bbad = raw[_DCC_DEST_ADDRESS]
        self.src_bank = raw[_DCC_SRC_BANK]
        self.hdma_bank = raw[_DCC_HDMA_BANK]
        self.hdma_line_counter = raw[_DCC_HDMA_LINE_COUNTER]
        self.src_address = raw[_DCC_SRC_ADDRESS] | (raw[_DCC_SRC_ADDRESS + 1] << 8)
        self.transfer_size = (raw[_DCC_TRANSFER_SIZE]
                              | (raw[_DCC_TRANSFER_SIZE + 1] << 8))
        self.hdma_table_address = (raw[_DCC_HDMA_TABLE_ADDRESS]
                                   | (raw[_DCC_HDMA_TABLE_ADDRESS + 1] << 8))
        # Mesen2 SnesDmaController.cpp:536-543 — the $43x0 read handler.
        self.dmap = (
            (0x80 if self.invert_direction else 0)
            | (0x40 if self.indirect else 0)
            | (0x20 if self.unused_control_flag else 0)
            | (0x10 if self.decrement else 0)
            | (0x08 if self.fixed else 0)
            | (self.transfer_mode & 0x07)
        )

    def __repr__(self):
        return (f"SnesDmaChannel(ch={self.ch}, dmap=${self.dmap:02X}, "
                f"bbad=${self.bbad:02X} -> ${0x2100 | self.bbad:04X})")


class SnesDmaState:
    """The live DMA controller: 8 channels plus the real HDMAEN ($420C) mask."""

    __slots__ = ("channels", "hdmaen")

    def __init__(self, channels, hdmaen: int):
        self.channels = tuple(channels)
        self.hdmaen = hdmaen

    @property
    def enabled_channels(self) -> set:
        """The channel numbers HDMAEN actually arms in silicon."""
        return {b for b in range(8) if self.hdmaen & (1 << b)}

    def __repr__(self):
        return f"SnesDmaState(hdmaen=${self.hdmaen:02X}, {self.channels})"


class SnesInternalRegs:
    """The live internal-register state, incl. a reconstructed NMITIMEN byte.

    `nmitimen` is rebuilt from the four enable flags Mesen keeps (bits 7/5/4/0
    of $4200 — InternalRegisters.cpp:299-327). Bits 1-3 and 6 are unused on
    hardware and read back as 0 here.
    """

    __slots__ = ("nmitimen", "enable_nmi", "enable_auto_joypad_read",
                 "enable_fast_rom", "enable_h_irq", "enable_v_irq",
                 "h_timer", "v_timer", "io_port_output")

    def __init__(self, raw: bytes):
        self.enable_auto_joypad_read = bool(raw[_IR_AUTO_JOYPAD])
        self.enable_fast_rom = bool(raw[_IR_FAST_ROM])
        self.enable_nmi = bool(raw[_IR_ENABLE_NMI])
        self.enable_h_irq = bool(raw[_IR_ENABLE_H_IRQ])
        self.enable_v_irq = bool(raw[_IR_ENABLE_V_IRQ])
        self.h_timer = raw[_IR_H_TIMER] | (raw[_IR_H_TIMER + 1] << 8)
        self.v_timer = raw[_IR_V_TIMER] | (raw[_IR_V_TIMER + 1] << 8)
        self.io_port_output = raw[_IR_IO_PORT_OUTPUT]
        self.nmitimen = (
            (0x80 if self.enable_nmi else 0)
            | (0x20 if self.enable_v_irq else 0)
            | (0x10 if self.enable_h_irq else 0)
            | (0x01 if self.enable_auto_joypad_read else 0)
        )

    def __repr__(self):
        return (f"SnesInternalRegs(nmitimen=${self.nmitimen:02X}, "
                f"fast_rom={self.enable_fast_rom})")


class SnesSnapshot:
    """One instant of machine state, all fields from a single GetConsoleState."""

    __slots__ = ("master_clock", "dma", "internal_regs", "cpu_a", "cpu_pc",
                 "cpu_k", "emulation_mode")

    def __init__(self, master_clock, dma, internal_regs, cpu_a, cpu_pc=None,
                 cpu_k=None, emulation_mode=None):
        self.master_clock = master_clock
        self.dma = dma
        self.internal_regs = internal_regs
        self.cpu_a = cpu_a
        self.cpu_pc = cpu_pc
        self.cpu_k = cpu_k
    ...
    @property
    def site(self) -> tuple:
        """(program bank, PC) — the code address the CPU is stopped on.

        At a write breakpoint this identifies the STORE SITE, which is what
        distinguishes two claims whose channel encoding is identical
. Note the 65816 has already
        advanced PC past the operand when the write is observed, so treat this
        as an opaque site identity, not as the instruction's own address.
        """
        return (self.cpu_k, self.cpu_pc)

    def __repr__(self):
        return (f"SnesSnapshot(clock={self.master_clock}, "
                f"A=${self.cpu_a:04X}, site=${self.cpu_k:02X}:"
                f"${self.cpu_pc:04X}, {self.internal_regs}, {self.dma})")


def _validate_snes_state_layout(buf: bytes) -> None:
    """Reject a GetConsoleState buffer that does not fit the mirrored layout.

    Purely structural, so it needs no knowledge of the ROM under test: C++
    `bool` reads back as exactly 0 or 1, DMAP's transfer mode is three bits,
    and the H/V IRQ timers are 9-bit hardware registers. A wrong offset lands
    in SnesPpuState / SpcState / DspState, where those constraints break almost
    immediately — which is the point: a silently-misaligned ctypes read that
    happens to look plausible would be a worse bug than the one this API exists
    to fix.
    """
    cpu = buf[_OFF_SNES_CPU_A:_OFF_SNES_CPU_A + 22]
    for off in _CPU_BOOL_OFFSETS:
        if cpu[off] > 1:
            raise SnesStateLayoutError(
                f"GetConsoleState: SnesCpuState bool field at A+{off} reads "
                f"{cpu[off]} (a C++ bool is 0 or 1) — the mirrored SnesState "
                f"layout does not match this Mesen2 core. Re-derive with "
                f"tools/mesen_state_offsets.cpp.")
    if cpu[_CPU_OFF_STOP_STATE] > 2:
        raise SnesStateLayoutError(
            f"GetConsoleState: SnesCpuState StopState reads "
            f"{cpu[_CPU_OFF_STOP_STATE]} (the enum is 0/1/2) — the mirrored "
            f"SnesState layout does not match this Mesen2 core. Re-derive "
            f"with tools/mesen_state_offsets.cpp.")
    for ch in range(8):
        base = _OFF_SNES_DMA + ch * _DMA_CH_STRIDE
        raw = buf[base:base + _DMA_CH_STRIDE]
        if raw[_DCC_TRANSFER_MODE] > 7:
            raise SnesStateLayoutError(
                f"GetConsoleState: DMA channel {ch} TransferMode is "
                f"{raw[_DCC_TRANSFER_MODE]} (hardware DMAP mode is 3 bits, "
                f"0-7) at offset ${base + _DCC_TRANSFER_MODE:04X} — the "
                f"mirrored SnesState layout does not match this Mesen2 core. "
                f"Re-derive the offsets with tools/mesen_state_offsets.cpp.")
        for off in _DCC_BOOL_OFFSETS:
            if raw[off] > 1:
                raise SnesStateLayoutError(
                    f"GetConsoleState: DMA channel {ch} bool field at "
                    f"+{off} reads {raw[off]} (a C++ bool is 0 or 1) — the "
                    f"mirrored SnesState layout does not match this Mesen2 "
                    f"core. Re-derive with tools/mesen_state_offsets.cpp.")
    ir = buf[_OFF_SNES_INTERNAL_REGS:_OFF_SNES_INTERNAL_REGS + 20]
    for off in _IR_BOOL_OFFSETS:
        if ir[off] > 1:
            raise SnesStateLayoutError(
                f"GetConsoleState: InternalRegs bool field at +{off} reads "
                f"{ir[off]} (a C++ bool is 0 or 1) — the mirrored SnesState "
                f"layout does not match this Mesen2 core. Re-derive with "
                f"tools/mesen_state_offsets.cpp.")
    for name, off in (("HorizontalTimer", _IR_H_TIMER),
                      ("VerticalTimer", _IR_V_TIMER)):
        v = ir[off] | (ir[off + 1] << 8)
        if v > 0x1FF:
            raise SnesStateLayoutError(
                f"GetConsoleState: InternalRegs {name} reads ${v:04X} (the "
                f"hardware register is 9 bits) — the mirrored SnesState "
                f"layout does not match this Mesen2 core. Re-derive with "
                f"tools/mesen_state_offsets.cpp.")


# --- Debugger breakpoints (Core/Debugger/Breakpoint.h) ------------------------
#
# `Breakpoint` is a POD-ish class (`char _condition[1000]`, no std::string), so
# it mirrors cleanly. Offsets derived the same way as SnesState's — see
# tools/mesen_state_offsets.cpp; sizeof(Breakpoint) = 1028 is asserted below so
# a layout change is loud rather than silent.
_BP_TYPE_READ = 1                   # BreakpointTypeFlags::Read
_BP_TYPE_WRITE = 2                  # BreakpointTypeFlags::Write
_BP_TYPE_EXECUTE = 4                # BreakpointTypeFlags::Execute


class _Breakpoint(ctypes.Structure):
    """Mirrors Core/Debugger/Breakpoint.h (private fields, in declared order)."""
    _fields_ = [
        ("Id", ctypes.c_uint32),
        ("CpuType", ctypes.c_uint8),
        ("_pad", ctypes.c_uint8 * 3),
        ("MemType", ctypes.c_int32),         # enum class MemoryType : int
        ("Type", ctypes.c_int32),            # enum class BreakpointTypeFlags
...
_BREAKPOINT_SIZEOF = 1028       # Mesen2's Breakpoint, per tools/mesen_state_offsets.cpp


class MesenAbiMismatch(RuntimeError):
    """A ctypes mirror in this file no longer matches the Mesen2 core's struct.

    Raised at the point of USE, never at import: a bare
    module-level `assert` on sizeof() meant a Mesen2 bump broke every test that
    merely imports the runner — including the ones that read memory and never
    touch a breakpoint — instead of only the breakpoint tests it actually
    invalidates. The blast radius should match the damage.
    """
...
# the S7 M1 work item report; summary:
#
# _CPU_TYPE_SNES = 0
...
#       9 -> ran away, never stopped (RunToIrq; the ROMs here use no IRQ)
...
# How long the emulator may make NO scanline progress before `_await_step`
# calls it stalled. This is a stall guard, not a budget: every observed
# scanline pushes the deadline out by this much, so a step on a starved host
# takes longer and still completes, while a lost Step request that no reissue
# recovers still fails in bounded time. See `_await_step`'s docstring.
#
# 30 s is `wait_frames`'s `stall_s`, deliberately: that parameter answers the
# same question ("has the machine stopped, or is the host merely slow?") and
# a second opinion about the same threshold in the same file is how the two
# drift. It was 5 s for one commit and that was measurably too tight — under
# 12 spinners on 4 cores (3x oversubscription) the emulation thread really
# does go 5 s without a scanline, and the guard then failed live-but-starved
# steps. The cost of the larger value is paid only on genuine wedges, and a
# wedge has ~600 reissue attempts to recover in before it fires.
_STEP_STALL_S = 30.0
...
def _detect_core_path() -> str:
    ...
    ``SF_MESEN_CORE``, when set to an existing file, wins outright on every
    platform — the per-worktree binding knob for cores built from
    ``vendor/mesen_patches/`` (the lockstep core lives in a PRIVATE tree
    because /tmp/Mesen2 is shared with other sessions on this box and must
    not be rebuilt in place). Additive: unset / empty / missing-file falls
    through to the historical candidate order unchanged.
    """
    ...
    if system == "Windows":
        ...
        candidates = [
            # Repo-local tools directory (this file lives at <repo>/vendor/,
            # so the repo root is two parents up — NOT three. Three resolves to
            # the directory ABOVE the repo root, which is somebody else's.)
            str(Path(__file__).resolve().parent.parent / "tools" / "Mesen" / "MesenCore.dll"),
            # User's AppData
        ...
        candidates = [
            ...
            # Repo-local cache — see the Windows branch above for why this is
            # two parents up rather than three.
            str(Path(__file__).resolve().parent.parent / "tools" / "Mesen" / "MesenCore.so"),
...
def _bind_global_functions(lib: ctypes.CDLL) -> None:
    ...
    # --- Live console state + breakpoints -------------------------------------
    # GetConsoleState is the only route to the DMA controller's real registers
    # (the peek path returns open bus for $4300-$431F / $420C). Bound
    # defensively; snes_dma_state() raises a clear RuntimeError when absent.
    if hasattr(lib, "GetConsoleState"):
        lib.GetConsoleState.restype = None
        # (BaseState& state, ConsoleType consoleType) — the reference is a
        # plain pointer to a caller-owned buffer the core memcpy-fills.
        lib.GetConsoleState.argtypes = [ctypes.c_void_p, ctypes.c_uint32]
    if hasattr(lib, "SetBreakpoints"):
        lib.SetBreakpoints.restype = None
        # (Breakpoint* breakpoints, uint32 count)
        lib.SetBreakpoints.argtypes = [ctypes.c_void_p, ctypes.c_uint32]
    if hasattr(lib, "GetProgramCounter"):
    ...
    # Emu-level pause API (uninit-detector power-on arming). With a debugger
    # attached, Pause() routes to Debugger::Step(cpu, 1, Step, Pause) and
    # IsPaused() to Debugger::IsPaused() — the exact predicate
    # Emulator::InternalLoadRom reads as `wasPaused` when deciding to plant a
    # first-instruction break on a reload (Mesen2 Core/Shared/Emulator.cpp).
    if hasattr(lib, "Pause"):
        lib.Pause.restype = None
        lib.Pause.argtypes = []
    if hasattr(lib, "IsPaused"):
        lib.IsPaused.restype = ctypes.c_bool
        lib.IsPaused.argtypes = []
...
#                                    green until a rollout is sequenced
#                                    deliberately, module by module.
...
def _apply_poweron_state(cfg: "_SnesConfig") -> None:
    ...
    if mode in ("", "0", "default"):
        ...
        # here — flipping it at once would turn every latch-sensitive module
        # red in the same run, with nothing to tell them apart.
        return
...
def _make_base_snes_config(force_random_latches: bool = False) -> "_SnesConfig":
    ...
    # RENDER EVERY FRAME — mechanism in the block above
    # `_apply_free_run_speed`. This is the shape with a concluded-GREEN CI
    # run behind it (30712937845, 8:24, +11%); the cheaper per-capture
    # scoping was tried on 87c1a91 and that run went red, so the cost is
    # paid rather than gambled.
...
def _global_initialize(
    ...
    # last so the env knob wins when set. See
...
def _global_atexit() -> None:
    """Quiesce MesenCore at Python interpreter exit.
    ...
    to run before interpreter shutdown — and if it doesn't, the process
    ...
    try:
        ...
        # During interpreter shutdown ctypes may raise on attribute access
...
class MesenRunner:
    ...
    def __init__(
        ...
        # FREE-RUN SPEED POLICY. With MaximumSpeed lifted the
        # core free-runs at ~150 fps instead of 60 — measured, see the
        # _EMU_FLAG_MAXIMUM_SPEED comment block. That is safe for every wait
        # in this harness because they are all counted in EMULATED frames
        # (wait_frames / wait_until / frame_step), so a faster core just
        # arrives sooner.
        #
        # The ONE exception is audio, and it is not a scheduling detail: a WAV
        # capture is a recording of real time, so an emulator running at 2.5x
        # produces 2.5x the samples for the same nominal frame count and every
        # duration assertion over it moves. Audio runners therefore keep the
        # 60 fps throttle, and tests/test_slice_b_audio.py's header says so at
        # the point of use.
        self._free_run_max_speed = not enable_audio
        ...
        # Scratch buffer for the single-address write_count() form, ditto.
        self._one_counter_buf = None
    ...
    # THE LAST WALL CLOCK IN THE HARNESS LIVES INSIDE THE EMULATOR.
    # SnesPpu.cpp:491 sets `_skipRender` when
    #
    #     !DisableFrameSkipping && GetEmulationSpeed() == 0 (unthrottled)
    #     && _frameSkipTimer.GetElapsedMS() < 10
    #
    # so a FREE-RUNNING core renders roughly one frame in three. SendFrame
    # still hands the video decoder a frame every time, but `_currentBuffer`
    # was never re-rendered — and TakeScreenshot copies that buffer. The PNG
    # could therefore be several EMULATED frames stale, with a wall-clock
    # timer deciding which frames survived. Measured on room.sfc, capturing
    # every frame of a fade and comparing the PNG's peak brightness to the
    # fade level in WRAM: unthrottled gave repeats and a REVERSAL (level
    # 0 -> 2 while the peak went 66 -> 0) and ended at level 15 showing
    # 231/255; throttled gave a distinct, monotone step every frame. That
    # 231-at-level-15 was CI run 30710534698's red.
    #
    # WHAT SHIPS: `DisableFrameSkipping = True`, PROCESS-WIDE, set once in
    # `_make_base_snes_config`. The first clause of the condition above is
    # then false for the life of the process, so every frame is rendered at
    # every emulator speed and the throttle stops having any bearing on what
    # a PNG contains. THIS METHOD — the throttle seam — therefore does
    # nothing for capture correctness, and does not need to: that is exactly
    # what makes free-running safe for the ~15 sites that photograph
    # something.
    #
    # The scope is process-wide because no amount of settling in a caller can
    # compensate for a frame the emulator never re-rendered; the guarantee has
    # to be structural. It costs about +11%, spread over every frame_step
    # drive in the suite (CI 30712937845: whole run 8:24 against a 7:34
    # baseline), and that is paid deliberately. Pinned by
    # tests/test_wait_primitives.py::test_the_process_renders_every_frame,
    # which asserts on the config the harness applies.
    #
    # Two cheaper per-capture scopings were tried and neither held. The
    # numbers, the misread CI figure that motivated the attempt, and the
    # reason each was rejected are in under the
    # dx-suite-time section — not repeated here, because a comment states the
    # constraint the code is under, not the route it took to get there.

    def _apply_free_run_speed(self) -> None:
        """Put the core at this runner's free-run speed policy.

        One place decides it, and both the load path and the resume path
        route through here — otherwise `debug_resume` restoring a hardcoded
        `False` would silently drop every frame-stepping test back to 60 fps
        the moment it handed the runner on.
        """
        if self._lib is not None and hasattr(self._lib, "SetEmulationFlag"):
            self._lib.SetEmulationFlag(_EMU_FLAG_MAXIMUM_SPEED,
                                       self._free_run_max_speed)
    ...
    def set_power_on_random_latches(self, enable: bool) -> None:
        ...
        ```` and
        the power-on fidelity convention.
        """
    ...
    def _require_uninit_arming_api(self) -> None:
        """Raise if the core lacks the exports the power-on arming needs."""
        missing = [
            name for name in
            ("Pause", "IsPaused", "IsExecutionStopped", "ResumeExecution")
            if not hasattr(self._lib, name)
        ]
        if missing:
            raise RuntimeError(
                "This MesenCore build lacks the pause/step exports required "
                f"to arm access counting from power-on: {missing}. Without "
                "them the counters attach mid-boot and the detector misses "
                "boot-time writes (false uninit-read positives). Rebuild "
                "Mesen2 from a recent SourMesen/Mesen2 checkout."
            )
    ...
    def load_rom_with_uninit_detection(
        ...
        Unlike :meth:`load_rom` (which inits the debugger *after* the run),
        this guarantees the counters are live before the ROM's FIRST
        instruction. That is mandatory for the uninitialized-read detector: a
        byte's "was it written before it was read" history is only complete if
        the counters observed the whole run. Call
        :meth:`get_uninitialized_reads` or
        :meth:`assert_no_uninitialized_reads` afterwards.

        Boot budget: pass ``frames=N`` to let the ROM run N EMULATED frames
        (the deterministic form — see :meth:`wait_frames`). ``run_seconds``
        is the wall-clock legacy form and buys a host-load-dependent, and
        under free-run throttle-dependent, number of frames.

        Mechanism (verified against Mesen2 Core/Shared/Emulator.cpp — LoadRom
        starts the emulation thread immediately, so a plain load + attach
        RACES the boot: writes that land before the debugger attaches are
        missed, and every later read of those bytes is a false "read-before-
        write". The flagged set is then exactly the written-once-at-boot,
        read-every-frame bytes, and varies with host timing):

        1. Load the ROM once and attach the debugger — this instance is a
           throwaway; its counter history doesn't matter.
        2. Pause(). With a debugger attached this routes to
           Debugger::Step(cpu, 1, Step, Pause) and execution stops.
        3. Load the SAME ROM again. InternalLoadRom reads ``wasPaused =
           IsPaused()`` before tearing down, builds a fresh console + fresh
           debugger (zero-initialized MemoryAccessCounter), and — because
           ``wasPaused && _debugger`` — plants a first-instruction break
           BEFORE the new emulation thread starts. Power-on RAM is
           re-randomized by this load like any other (SF_HW_POWERON regime).
        4. Wait for the planted break, then ResumeExecution(). Every access
           from the reset vector onward is counted; nothing ran unobserved
           (the one instruction executed by the planted break is counted
           too — counting is not gated on the break).
        """
        self._ensure_initialized()
        self._require_access_counter_api()
        self._require_uninit_arming_api()
        abs_path = os.path.abspath(rom_path)
        if not os.path.exists(abs_path):
            raise FileNotFoundError(f"ROM not found: {abs_path}")
        ...
        # --- 1. Throwaway load: exists only so a debugger can attach --------
        loaded = self._lib.LoadRom(abs_path.encode("utf-8"), None)
        if not loaded:
            raise RuntimeError(f"Mesen2 failed to load ROM: {abs_path}")
        self._rom_loaded = True
        ...
        # --- 2. Break it. Pause() == a debugger-routed 1-instruction step.
        # Reissue on a 200 ms no-stop stall (same lost-wakeup class the
        # frame-step API recovers from; a re-Pause on a running emu just
        # re-requests the break — safe).
        deadline = time.time() + 10.0
        self._lib.Pause()
        last_issue = time.time()
        while not (self._lib.IsPaused() and self._lib.IsExecutionStopped()):
            now = time.time()
            if now > deadline:
                raise RuntimeError(
                    "uninit-detector arming: emulator did not stop after "
                    "Pause() within 10 s")
            if now - last_issue > 0.2:
                self._lib.Pause()
                last_issue = now
            time.sleep(0.002)

        # --- 3. Reload while paused: fresh console, fresh (zeroed) counters,
        # first-instruction break planted before the emu thread starts.
        loaded = self._lib.LoadRom(abs_path.encode("utf-8"), None)
        if not loaded:
            raise RuntimeError(f"Mesen2 failed to re-load ROM: {abs_path}")

        deadline = time.time() + 10.0
        while not self._lib.IsExecutionStopped():
            if time.time() > deadline:
                raise RuntimeError(
                    "uninit-detector arming: planted power-on break never "
                    "landed within 10 s of reload")
            time.sleep(0.002)

        # --- 4. Run for real, counters live from the reset vector. ----------
        # (No ResetMemoryAccessCounts here: the reload built a fresh zeroed
        # counter set, and a reset NOW would erase the already-counted
        # accesses of the one instruction the planted break executed.)
        self._apply_free_run_speed()
        self._lib.ResumeExecution()
        self._frame_stepping = False
        if frames is not None:
            # Emulated-frame boot budget — the deterministic form. See
            # wait_frames / the de-flake block comment.
            self.wait_frames(frames)
        else:
            time.sleep(run_seconds)

    # --- Live hardware state (DMA controller + internal registers) ------------
    #
    # THE point of these: the DMA controller's registers are invisible to the
    # debugger's peek path, so before this existed the only way to check "which
    # HDMA channels are armed" was the ROM's own WRAM shadow — one copy removed
    # from the silicon, and blind to a channel armed directly in $420C.

    def _require_console_state_api(self) -> None:
        if not hasattr(self._lib, "GetConsoleState"):
            raise RuntimeError(
                "This MesenCore build does not export GetConsoleState — the "
                "live DMA/internal-register state needs a Mesen2 core with the "
                "debugger console-state API (InteropDLL/DebugApiWrapper.cpp).")
        if not self._debugger_active:
            raise RuntimeError(
                "GetConsoleState routes through the debugger (WithDebugger); "
                "load a ROM first so the debugger is initialized.")

    def snes_console_state(self) -> bytes:
        """Raw `SnesState` bytes for the running SNES (layout validated).

        Prefer :meth:`snes_dma_state` / :meth:`snes_internal_regs` /
        :meth:`snes_state_snapshot`; this is the escape hatch for reaching a
        field the wrappers do not expose yet.
        """
        self._require_console_state_api()
        buf = (ctypes.c_uint8 * _SNES_STATE_BUF_BYTES)()
        self._lib.GetConsoleState(ctypes.cast(buf, ctypes.c_void_p),
                                  _CONSOLE_TYPE_SNES)
        raw = bytes(buf)
        _validate_snes_state_layout(raw)
        return raw

    def _dma_state_from(self, raw: bytes) -> SnesDmaState:
        chans = [
            SnesDmaChannel(
                ch, raw[_OFF_SNES_DMA + ch * _DMA_CH_STRIDE:
                        _OFF_SNES_DMA + (ch + 1) * _DMA_CH_STRIDE])
            for ch in range(8)
        ]
        return SnesDmaState(chans, raw[_OFF_SNES_DMA + _DMA_OFF_HDMAEN])

    def snes_dma_state(self) -> SnesDmaState:
        """The live DMA controller: all 8 channels' registers + real HDMAEN.

        `state.hdmaen` is the actual `$420C` mask the silicon is acting on —
        not a WRAM shadow of it. `state.channels[n].dmap` / `.bbad` are the two
        bytes an hdma / dma_init declaration emits, so they compare directly
        against ES_H_<CLAIM>_DMAP / ES_H_<CLAIM>_BBAD.
        """
        return self._dma_state_from(self.snes_console_state())

    def snes_internal_regs(self) -> SnesInternalRegs:
        """The live internal registers, incl. a reconstructed NMITIMEN byte."""
        raw = self.snes_console_state()
        return SnesInternalRegs(
            raw[_OFF_SNES_INTERNAL_REGS:_OFF_SNES_INTERNAL_REGS + 20])

    def snes_cpu_a(self) -> int:
        """The 65816 accumulator (full 16-bit C register)."""
        raw = self.snes_console_state()
        return raw[_OFF_SNES_CPU_A] | (raw[_OFF_SNES_CPU_A + 1] << 8)

    def snes_state_snapshot(self) -> SnesSnapshot:
        """DMA state + internal registers + A, all from ONE GetConsoleState call.

        Use this — not three separate accessors — whenever the readings must be
        mutually consistent, i.e. any time you are reasoning about *one* instant.
        `IsExecutionStopped()` can read True while the emulation thread is still
        finishing the instruction the break landed on, so back-to-back accessor
        calls around a breakpoint hit can straddle two different machine states
        (observed: an MDMAEN mask from one instant paired with channel registers
        from another, which made a single-channel fire look like a seven-channel
        one). One buffer, one instant.
        """
        raw = self.snes_console_state()
        return SnesSnapshot(
            master_clock=int.from_bytes(raw[0:8], "little"),
            dma=self._dma_state_from(raw),
            internal_regs=SnesInternalRegs(
                raw[_OFF_SNES_INTERNAL_REGS:_OFF_SNES_INTERNAL_REGS + 20]),
            cpu_a=raw[_OFF_SNES_CPU_A] | (raw[_OFF_SNES_CPU_A + 1] << 8),
            cpu_pc=(raw[_OFF_SNES_CPU_A + _CPU_OFF_PC]
                    | (raw[_OFF_SNES_CPU_A + _CPU_OFF_PC + 1] << 8)),
            cpu_k=raw[_OFF_SNES_CPU_A + _CPU_OFF_K],
            emulation_mode=bool(raw[_OFF_SNES_CPU_A
                                    + _CPU_OFF_EMULATION_MODE]),
        )

    # --- Debugger breakpoints -------------------------------------------------

    def _require_breakpoint_api(self) -> None:
        if ctypes.sizeof(_Breakpoint) != _BREAKPOINT_SIZEOF:
            raise MesenAbiMismatch(
                f"_Breakpoint mirror is {ctypes.sizeof(_Breakpoint)} bytes, "
                f"Mesen2's Breakpoint is {_BREAKPOINT_SIZEOF} — SetBreakpoints "
                f"would read past the array. Re-derive with "
                f"tools/mesen_state_offsets.cpp.")
        for name in ("SetBreakpoints", "ResumeExecution", "IsExecutionStopped"):
            if not hasattr(self._lib, name):
                raise RuntimeError(
                    f"This MesenCore build does not export {name} — the "
                    f"breakpoint API needs a Mesen2 core with the debugger "
                    f"stepping exports.")

    def set_breakpoints(self, specs) -> None:
        """Install write/read/exec breakpoints; `specs` = [(mem_type, addr, kind)].

        `kind` is one of "write" / "read" / "exec". Pass an empty list to clear.
        Addresses are interpreted in *mem_type*'s own address space — use
        MemoryType.SnesMemory with a CPU-bus address (e.g. 0x420B) to break on a
        register write regardless of which bank the store used.
        """
        self._require_breakpoint_api()
        kinds = {"write": _BP_TYPE_WRITE, "read": _BP_TYPE_READ,
                 "exec": _BP_TYPE_EXECUTE}
        if not specs:
            self._lib.SetBreakpoints(None, 0)
            return
        arr = (_Breakpoint * len(specs))()
        for i, (mem_type, addr, kind) in enumerate(specs):
            if kind not in kinds:
                raise ValueError(f"breakpoint kind must be one of "
                                 f"{sorted(kinds)}, got {kind!r}")
            bp = arr[i]
            bp.Id = i + 1
            bp.CpuType = _CPU_TYPE_SNES
            bp.MemType = int(mem_type)
            bp.Type = kinds[kind]
            bp.StartAddr = addr
            bp.EndAddr = addr
            bp.Enabled = True
            bp.MarkEvent = False
            # Dummy reads/writes are the CPU's internal address-bus artifacts,
            # not program stores — matching them would break on instructions
            # that never touch the register.
            bp.IgnoreDummyOperations = True
            bp.Condition = b""
        self._lib.SetBreakpoints(ctypes.cast(arr, ctypes.c_void_p), len(specs))

    @contextlib.contextmanager
    def breakpoints(self, specs):
        """Context manager: install `specs`, clear them (and resume) on exit.

        On exit the emulator is returned to free-running mode, so a test that
        raises mid-block cannot leave a shared runner parked at a break.
        """
        self.set_breakpoints(specs)
        if hasattr(self._lib, "SetEmulationFlag"):
            self._lib.SetEmulationFlag(_EMU_FLAG_MAXIMUM_SPEED, True)
        try:
            yield self
        finally:
            self.set_breakpoints([])
            # A break leaves the emulation thread stopped; debug_resume() only
            # acts when it thinks it parked the machine, so mark it parked.
            self._frame_stepping = True
            self.debug_resume(clear_input=False)

    def _master_clock(self) -> int:
        """The emulator's master-clock counter (SnesState.MasterClock)."""
        return int.from_bytes(self.snes_console_state()[0:8], "little")

    def _settle_at_break(self, tries: int = 400, attempts: int = 4,
                         lost_after_s: float = 0.050) -> None:
        """Wait until the master clock stops moving, i.e. the break has landed.

        `IsExecutionStopped()` goes True as soon as the break is *requested*;
        the emulation thread may still be running, so state read immediately can
        be self-inconsistent (measured: ~5 in 1200 breakpoint hits returned CPU
        and DMA registers from different instants before this settle existed).
        Two consecutive equal clock readings 1 ms apart mean the machine really
        is parked — one frame is ~357k master cycles, so any motion at all shows
        up as a difference.

        The two-consecutive-equal invariant is the correct one and is unchanged.
        What changed is how a NON-settling machine is handled, and the history is
        worth keeping because the first fix treated the wrong hypothesis:

        - Original: 200 tries (~200 ms), message with no numbers. Timed out once
          in a full-suite run while passing 3/3 standalone.
        - First fix (2026-07-28): budget → 1500 tries, plus drift/elapsed/frame
          reporting. That was a guess at "short budget under host load", and the
          instrumentation is the only part that earned its keep.
        - What the instrumentation then showed, on the next occurrence:
          **193,145,848 master cycles — 540.5 frames — over 1.500 s across all
          1500 tries.** The machine was not "about to park"; it was running
          continuously the entire time. `IsExecutionStopped()` goes True as soon
          as a break is *requested*, so a request the emulation thread never
          completes leaves the flag stuck True with the clock free-running, and
          no budget however large will ever see two equal readings.

        So the fix is not a longer wait, it is a RE-REQUEST. If the clock is
        still advancing after `lost_after_s` of continuous motion — far past any
        real park latency, which is sub-millisecond — the break request was lost.
        Re-issue it and wait again, bounded by `attempts`. Only a machine that
        ignores several consecutive requests is a real failure.

        Reproduction rate is low (2 occurrences, both inside a full `make test`;
        5/5 clean standalone), so treat a recurrence with a SMALL drift figure as
        a genuinely different bug from this one and do not reach for the budget.
        """
        for attempt in range(attempts):
            start = last = self._master_clock()
            stable = 0
            moving_since = None
            waited = 0.0
            for _ in range(tries):
                ...
                if cur == last:
                    ...
                    if moving_since is None:
                        moving_since = waited
                    elif waited - moving_since >= lost_after_s:
                        break               # request lost — stop polling, re-ask
                last = cur
            drift = last - start
            if attempt + 1 < attempts:
                # The flag says stopped and the clock disagrees, so the request
                # was lost: ask again. Step(cpu, 1, PPU_SCANLINE) is the same
                # primitive _park_at_canonical_scanline uses, so a re-request
                # lands the machine at a scanline boundary exactly as the
                # original break would have — it does not invent a new park
                # semantics, and one scanline of slip is inside the tolerance of
                # every caller (the cost probe's band is 40..220 mc/iteration).
                self._lib.Step(_CPU_TYPE_SNES, 1, _STEP_TYPE_PPU_SCANLINE)
        raise TimeoutError(
            f"break did not settle after {attempts} break requests: the master "
            f"clock kept advancing while IsExecutionStopped() reported stopped. "
            f"Last attempt advanced {drift} master cycles "
            f"(~{drift / 357368:.1f} frames) over {waited:.3f}s. A LARGE frame "
            f"count means the emulation thread is ignoring break requests "
            f"outright — this is the known signature, see the docstring; a SMALL "
            f"one is a different bug and the budget is not the answer.")

    def run_to_break(self, timeout_s: float = 10.0,
                     max_frames: Optional[int] = None) -> bool:
        """Resume (if stopped) and block until a breakpoint stops execution.

        Returns True on a settled break, False if the budget elapsed without
        one. Requires breakpoints installed by :meth:`set_breakpoints`.

        BUDGET, and which one to use. `timeout_s` is a WALL deadline: the
        thing it is waiting for (the CPU reaching a store) advances in
        emulated time, so under host load the deadline can expire after far
        fewer emulated frames than the caller meant to give it, and the
        caller reads a False that means "the host was busy" rather than
        "the ROM did not write". That is the suite's flake mechanism
 in its `run_to_break` form.

        Pass `max_frames` instead and the budget is counted in EMULATED
        frames off the PPU counter, so "no write in 90 frames" means exactly
        that on an idle box and on a hammered one. `timeout_s` then degrades
        to a stall guard: it is re-armed on every observed frame advance, so
        it can only fire on an emulator that is not advancing at all.
        A False under `max_frames` is a claim about the ROM.

        The "did the resume take?" question is answered with the master clock,
        not with IsExecutionStopped(): a freshly-issued ResumeExecution can
        still read `stopped` for a moment, and treating that as an immediate
        break would return the SAME break over and over — a loop that looks
        like it sampled N transfers while it sampled one. A break only counts
        once the clock has moved past the clock of the break we resumed from.

        A lost ResumeExecution is re-issued only after the machine has been
        CONTINUOUSLY stopped at the same clock for 50 ms — the same rule
        _await_step uses, and for the same reason inverted: an eager re-issue
        resumes PAST a break that had already landed, and the caller then reads
        state off a *running* machine.

        A True IS NOT A PROOF THAT A BREAKPOINT MATCHED — read this before
        asserting on a False. The break test above is "execution is stopped AND
        the master clock moved", and Mesen's `IsExecutionStopped()` is
        `_executionStopped || _emu->IsThreadPaused()`
        (Core/Debugger/Debugger.cpp:740). That second term goes true for things
        that are not breakpoint hits: `Emulator::WaitForLock` raises
        `_threadPaused` whenever another thread holds an EmulatorLock, and
        `DebugBreakHelper`'s constructor deliberately spins the emulation thread
        down for the duration of assorted debugger API calls. Catch the machine
        inside one of those windows on a free-running emulator and both halves
        of the test hold with nothing having matched — and then
        `_settle_at_break()` finds the clock still advancing, concludes the
        break request was lost, and issues the `Step` that MANUFACTURES the
        break it was supposed to be confirming. The stop lands at whatever PC
        the CPU had reached.

        Measured on the room ROM: 7 such reports in 270 idle reps (~2.6%), the
        stops at $00:$81DD / $00:$81AE / $00:$83E6 while the only real store to
        the watched byte is at $00:$8A87, and Mesen's own write counter for the
        byte unmoved in every one.

        This is invisible to a caller that asserts a break DID happen — it gets
        the real one moments later. It is fatal to a caller asserting one did
        NOT. Such a caller must gate the result on the output surface for its
        actual proposition: :meth:`write_count` for a write, VRAM/OAM bytes for
        a transfer. `run_to_break` is the stopwatch, not the oracle.
        """
        self._require_breakpoint_api()
        was_stopped = bool(self._lib.IsExecutionStopped())
        clock0 = self._master_clock() if was_stopped else None
        if was_stopped:
            self._lib.ResumeExecution()
        deadline = time.time() + timeout_s
        stalled_since = None
        frame0 = self.ppu_frame_count() if max_frames is not None else 0
        last_frame = frame0
        while time.time() < deadline:
            now = time.time()
            if self._lib.IsExecutionStopped():
                if clock0 is None or self._master_clock() != clock0:
                    self._settle_at_break()
                    self._frame_stepping = True
                    return True
                if stalled_since is None:
                    stalled_since = now
                elif now - stalled_since > 0.05:
                    self._lib.ResumeExecution()      # lost wakeup: re-arm
                    stalled_since = None
            else:
                stalled_since = None
            if max_frames is not None:
                cur = self.ppu_frame_count()
                if cur - frame0 >= max_frames:
                    return False                    # the EMULATED budget
                if cur != last_frame:
                    # Progress: push the wall deadline out. Under max_frames
                    # it is only a wedged-emulator guard, never a budget.
                    last_frame = cur
    ...
    def write_count(self, mem_type: MemoryType, addr: int) -> int:
        """Times the memory system has written ONE byte, since the debugger attached.

        The cheap single-address form of :meth:`get_access_counts` (~2 us vs
        ~190 ms for a 128 KB region, so it is usable inside a poll loop).

        WHAT IT IS FOR. This is the OUTPUT SURFACE for the proposition "the CPU
        wrote this byte" — the memory system's own tally, incremented at the
        store. :meth:`run_to_break` does NOT answer that question, and the
        difference is not academic: it reports a break by observing
        `IsExecutionStopped()` with a moved master clock, and Mesen's
        `IsExecutionStopped()` is `_executionStopped || _emu->IsThreadPaused()`
        (Core/Debugger/Debugger.cpp:740). The second term goes true for things
        that are not breakpoint hits — `Emulator::WaitForLock` raises
        `_threadPaused` whenever another thread holds an EmulatorLock, and
        `DebugBreakHelper` spins the emulation thread down for the duration of
        assorted debugger calls. A caller that needs "a write happened" — in
        particular one asserting a write did NOT happen — must ask here.
        Measured: 7 spurious breaks in 270 idle reps of the room's iris table,
        every one with this counter unmoved.

        ADDRESSING. `addr` indexes *mem_type*'s own space, so for
        `SnesWorkRam` it is a WRAM OFFSET and is bank-agnostic: WRAM $04A5 is
        one byte whether the store reached it as $00:04A5 or as $7E:04A5. A
        `SnesMemory` breakpoint on $00:04A5 is the narrower claim — it does not
        match the long-addressed store. Pair them deliberately.
        """
        self._require_access_counter_api()
        if not self._rom_loaded:
            raise RuntimeError("No ROM loaded. Call load_rom() first.")
        if self._one_counter_buf is None:
            self._one_counter_buf = (_AddressCounters * 1)()
        self._lib.GetMemoryAccessCounts(addr, 1, int(mem_type),
                                        self._one_counter_buf)
        return int(self._one_counter_buf[0].WriteCounter)
    ...
    def assert_no_uninitialized_reads(
        ...
        Opt-in gate a work item calls explicitly after
    ...
    # Attempts allowed to `_park_for_capture` before it gives up. The retry
    # is NOT a blanket "try again and hope" — it fires only on a MEASURED
    # miss (the park crossed a second frame boundary), its success condition
    # is reachable and was observed on 57 of 60 reps under 6 CPU burners, and
    # exhausting it raises with the numbers rather than returning a wrong
    # picture. Contrast the break-settle flake ,
    # where the retry's success condition was structurally unreachable and no
    # budget could ever have helped.
    _CAPTURE_PARK_ATTEMPTS = 4

    def _park_for_capture(self, timeout_s: float = 10.0) -> int:
        """Park the machine so the composite is the frame current at entry.

        WHY THIS EXISTS. `TakeScreenshot` copies `BaseVideoFilter`'s output
        buffer (`BaseVideoFilter.cpp:166-180`). Nothing on the calling thread
        fills that buffer: `SnesPpu::SendFrame` hands the finished frame to
        `VideoDecoder::UpdateFrame`, which — with `sync=false`, the live-play
        case (`SnesPpu.cpp:1522`) — only sets `_frameChanged` and signals
        `_waitForFrame` (`VideoDecoder.cpp:181-205`). The pixels are written
        later, by `DecodeThread` -> `DecodeFrame` -> `_videoFilter->SendFrame`,
        on a thread nobody here drives.

        And the counter this class waits on runs AHEAD of even the submit:
        `SnesPpu.cpp:479` does `_frameCount++` and only THEN calls
        `SendFrame()`. So `wait_frames(1)` returns at the instant the counter
        reads C+1 — before frame C has been handed to the decoder at all, let
        alone decoded. Capturing there photographs frame C-1 whenever the
        decode thread has not been scheduled yet, which is a pure wall-clock
        question. MEASURED on this box, room.sfc, against a ground truth taken
        from a frozen machine: **1 stale capture in 60 idle, 35 in 60 under 6
        CPU burners** (2% -> 58%). That is the whole flake — a capture path
        whose correctness was a race the host's load ran for us. It reached CI
        as `test_a_capture_shows_the_frame_that_was_current_when_it_was_taken`
        reversing at a fade turnaround under `make test XDIST=2`.

        WHAT FIXES IT. A parked emulator submits nothing, so the buffer
        converges on the last submitted frame and STAYS there — the wait
        becomes monotone instead of a race. Parking at the canonical scanline
        with the counter reading Q means frames up to Q-1 have been submitted
        and Q-1 is the only one still possibly in flight, so the composite
        converges on exactly frame Q-1. Latching C first and requiring
        Q == C+1 makes that frame C: the frame that was current at entry.

        The residual, stated honestly: there is NO exported way to observe
        that the decode thread has consumed a frame (`GetFrameCount` and
        `WaitForAsyncFrameDecode` are `VideoDecoder`-private, and nothing in
        `EmuApiWrapper.cpp` reaches them), so "converged" is not something
        this method can assert — only something the park makes inevitable.
        What it buys is that the window is no longer a race against a moving
        machine, and the park is not a token pause: reaching scanline 224 from
        the submit point emulates a whole frame, so the decode thread gets that
        long plus the step's polling. MEASURED residual, same oracle, 6 CPU
        burners: **2 non-converged captures in 550 reps (0.4%)**, against 58%
        for the unparked path.

        Deliberately NOT "fixed" with a settle sleep. The park already hands
        the decoder more than 10 ms; a residual that survives that is a
        STARVED thread, which no bounded wait can promise to outlast, and a
        budget picked to make a number go away is the shape this repo has
        already paid for twice. If this ever
        needs closing, close it with an observable — not a bigger number.

        Returns:
            The latched frame number — the frame the composite now holds.

        Raises:
            RuntimeError: the park kept crossing an extra frame boundary.
        """
        if not self._debugger_active or not hasattr(self._lib, "Step"):
            # No debug-stepping exports: nothing to park with. Fall back to
            # the historical path and stay free-running — a stale composite
            # is better than a hard failure on a core built without the
            # debugger, and _frame_step_lib() already explains the rebuild.
            self.wait_frames(1)
            return self.ppu_frame_count() - 1

        misses = []
        for _ in range(self._CAPTURE_PARK_ATTEMPTS):
            latched = self.ppu_frame_count()
            self.wait_frames(1)
            self.debug_break(timeout_s)
            parked = self.ppu_frame_count()
            if parked == latched + 1:
                return latched
            # The park crossed a boundary the latch did not account for, so
            # the composite is converging on a LATER frame than the caller's.
            # Not a stale picture — a picture from the future — and it jitters
            # exactly where the fade-walk test is sharpest. Retry rather than
            # ship the wrong frame.
            misses.append(parked - latched)
            self.debug_resume(clear_input=False)
        raise RuntimeError(
            "take_screenshot could not park on the caller's frame after "
            f"{self._CAPTURE_PARK_ATTEMPTS} attempts: the park landed "
            f"{misses} frames past the latch (wanted 1 every time). The "
            "machine is running far enough ahead of this thread that no "
            "frame can be named and photographed atomically; capture from a "
            "parked runner (debug_break/frame_step) instead."
        )
    ...
    def take_screenshot(self, output_path: Optional[str] = None,
        ...
        Args:
            ...
            settle_frames: Advance this many EMULATED frames BEFORE
                         capturing — for letting the GAME settle, not for
                         letting the picture catch up. The capture is
                         frame-synchronous on its own (see below), so 0 is
                         correct unless the ROM itself needs more frames.
                         Works in both execution modes: ``wait_frames``
                         free-running, ``frame_step`` parked; the count is
                         frames either way, never wall seconds.

        FRAME-SYNCHRONOUS: the returned PNG holds the pixels of the emulated
        frame current at call time, under any host load. The method absorbs
        the one-frame presentation lag itself and the process runs with frame
        skipping disabled, so a caller no longer has to know either. What it
        does NOT do is wait for the GAME — if the ROM needs ten more frames to
        finish a transition, ask for them with ``settle_frames`` or drive them
        yourself.

        (History, because the old advice was to hand-tune this: the note here
        used to tell callers to pass ``settle_frames=1`` for the presentation
        lag. That was right and insufficient — under free-run the PPU also
        SKIPS rendering, so no fixed number of settle frames was ever enough,
        and CI run 30710534698 failed on a title screen the emulator had
        already finished fading. The lag is absorbed here now and the skipping
        is off; hand-tuning a settle count is no longer a thing to do.)

        A FREE-RUNNING capture PARKS for the duration — see
        ``_park_for_capture``. That is what makes the guarantee hold under
        host load rather than only on an idle box; the park is transparent
        (input overrides survive, the runner is handed back free-running)
        and costs the same ONE emulated frame it always did. The one visible
        edge: an audio recording running across a capture gets a short gap,
        because a parked emulator produces no samples.
        ...
        # Optional pre-capture settle (default 0 => unchanged). Counted in
        # EMULATED frames in both execution modes: wait_frames() off the PPU
        # counter while free-running, frame_step() while parked. It used to
        # be run_frames(), i.e. a wall sleep that bought however many frames
        # the host happened to render — and NOTHING at all while parked, so
        # the shot was of the pre-settle frame. Being one frame early is
        # exactly the failure this argument exists to prevent.
        if settle_frames > 0:
            self._advance_emulated(settle_frames)

        # THE FRAME-SYNCHRONOUS GUARANTEE: the PNG holds the pixels of the
        # emulated frame that was current when you called, whatever the host
        # is doing. Two things stand between the caller and that, and they are
        # handled in two different places.
        #
        # FRAME SKIPPING is NOT handled here. It is suppressed process-wide by
        # `DisableFrameSkipping = True` in `_make_base_snes_config` — see the
        # long block above `_apply_free_run_speed` for the mechanism. Every
        # frame is rendered at every emulator speed, so this method touches
        # neither the flag nor the throttle.
        #
        # THE PRESENTATION LAG is handled here, and it is structural rather
        # than timing-dependent: SnesPpu::SendFrame hands the finished frame
        # to the video decoder at frame END, so at any instant the composite
        # holds the PREVIOUS completed frame. Bringing the frame that was
        # current at call time INTO the composite therefore COSTS an emulated
        # frame; there is no exported "re-composite what you already have"
        # primitive to borrow. The cost is paid unconditionally just below.
        ...
        # EXACTLY ONE emulated frame, in BOTH execution modes — `wait_frames`
        # free-running, `frame_step` parked (`_advance_emulated` picks). The
        # count is part of this method's contract and is load-bearing in both
        # directions:
        #
        #   zero — the composite still holds the frame BEFORE the caller's, so
        #     the PNG is off by one. That is the lag described above.
        #   two — one frame too many for a PARKED caller driving frames one at
        #     a time. Done unconditionally it moved the VWF row tests' "blank"
        #     reference two frames into the row's own render, and four
        #     differential assertions read ink starting 6 px late.
        #
        # So a parked caller pays one frame here as well; that is the price of
        # the guarantee holding in both modes, and it is small enough that no
        # caller hand-tunes a settle count to compensate. A caller that wants
        # more asks for it with `settle_frames`. Pinned by
        # tests/test_wait_primitives.py::test_a_capture_spends_exactly_one_emulated_frame.
        #
        # TakeScreenshot then writes the PNG on THIS thread
        # (EmuApiWrapper.cpp:171 calls VideoDecoder::TakeScreenshot directly,
        # which copies the output buffer under _frameLock), so the pixels are
        # fixed by the time it returns.
        #
        # WHICH pixels, though, is the question that lock does not answer.
        # `_frameLock` makes the copy atomic against a decode in progress; it
        # says nothing about WHICH FRAME is in the buffer, and the buffer is
        # written by a thread we do not drive. See `_park_for_capture` for the
        # ordering and the measurement. Free-running: park so the composite
        # can converge, capture, hand the machine back. Parked: the caller
        # already owns the machine, `frame_step` already left it stopped, and
        # `_advance_emulated`'s own polling has drained the decode — measured
        # 0 divergences in 40 reps under 6 CPU burners.
        parked_for_capture = False
        if self._frame_stepping:
            self._advance_emulated(1)
        else:
            self._park_for_capture()
            parked_for_capture = True
        self._lib.TakeScreenshot()
        if parked_for_capture:
            # Hand back a free-running machine, preserving any input overrides
            # the caller latched with set_input (clear_input=True would drop
    ...
    def start_audio_recording(self, output_path: str):
        ...
        Raises RuntimeError if execution is parked (debug_break /
        frame_step): a parked emulator generates no samples, so the WAV
        records SILENCE — an empty capture that looks like a test bug.
        Call debug_resume() first; the next frame_step() re-parks
        automatically.
        ...
        if not self._rom_loaded:
            raise RuntimeError("No ROM loaded. Call load_rom() first.")
        # Parked-recording guard. Recording while parked bit three separate
        # tests in one work item (the friction log "the audio slice — the sound of a
        # place"): the emulation thread is stopped, so WaveRecord captures
        # nothing until the eventual resume. Ground truth, NOT the
        # self._frame_stepping flag: the flag reads stale-True after a
        # timed-out run_to_break() (machine free-running, flag claiming
        # parked
        # flag-based guard refuse a legitimate recording; and it reads
        # stale-False when an installed breakpoint stopped the machine
        # without this class's bookkeeping seeing it, which would let a
        # silent capture through. IsExecutionStopped() is the emulator's own
        # answer and the predicate debug_break() itself consults for its
        # idempotence check. Double-read with a 1 ms settle (the
        # _park_at_canonical_scanline idiom) so the momentary stopped
        # reading right after a freshly-issued ResumeExecution (see
        # run_to_break's docstring) cannot raise falsely on a machine that
        # is actually taking off. Guard is skipped when the debugger is
        # inactive or the export is absent: debug_break/run_to_break both
        # require the export (via _frame_step_lib/_require_breakpoint_api),
        # so no park can exist through this class in either case.
        if self._debugger_active and hasattr(self._lib, "IsExecutionStopped"):
            if self._lib.IsExecutionStopped():
                time.sleep(0.001)
                if self._lib.IsExecutionStopped():
                    raise RuntimeError(
                        "start_audio_recording() while execution is parked: "
                        "a parked emulator records SILENCE (empty WAV). Call "
                        "debug_resume() first — the next frame_step() "
                        "re-parks automatically."
                    )
    ...
    def capture_frames(
        ...
        for target_frame in sorted_frames:
            ...
            if delta > 0:
                self.wait_frames(delta)   # EMULATED frames, not a wall sleep
    ...
    def _await_step(self, lib, done, reissue, deadline, what: str):
        ...
        THE DEADLINE IS A STALL GUARD, NOT A BUDGET — the same distinction
        `wait_frames`'s `stall_s` draws, and for the same reason. `deadline`
        arrives as a wall time computed from a frame count, but it is PUSHED
        FORWARD by `_STEP_STALL_S` every time the emulator demonstrably
        advances a scanline. So the question this loop asks is "has the
        machine stopped making progress?", never "has the machine finished
        inside a wall budget" — and a step that is merely SLOW because the
        host is starved cannot time out. It could before: a contended box
        raised `TimeoutError: frame walk timed out (frame 153, scanline 224)`
        out of `test_platformer.py::
        test_every_animation_frame_rests_on_the_surface_and_none_covers_it`,
        a test whose own loop is correctly bounded in emulated frames — the
        wall bound was underneath it, in here.

        A REISSUE DOES NOT COUNT AS PROGRESS. It resets the reissue throttle
        (`last_progress` below) but never the deadline, so a request that is
        being re-armed forever without the machine moving still times out.
        That asymmetry is what keeps this a guard rather than an immortality
        loop.
        """
        # Every attempt gets a full stall window of its own, whatever the
        # caller's clock says. `_step_to_target` computes ONE deadline and
        # passes it by value to a bulk step and then to N frame walks, so an
        # extension earned inside one call cannot reach the next — and the
        # last walk of a long `frame_step` would inherit an already-expired
        # deadline and fail instantly on a slow host. Measured: with this line
        # absent, `frame_step(60, timeout_s=0.3)` advanced all 60 frames and
        # then raised on the final walk. The max() keeps a caller's LARGER
        # budget intact.
        deadline = max(deadline, time.time() + _STEP_STALL_S)
        ...
        while time.time() < deadline:
            ...
            if pos != last_pos:
                ...
                last_progress = now
                stopped_since = None
                deadline = max(deadline, now + _STEP_STALL_S)
            ...
            time.sleep(0.0002)
        raise TimeoutError(
            f"{what}: the emulator made NO scanline progress for "
            f"{_STEP_STALL_S:g}s (frame {self.ppu_frame_count()}, scanline "
            f"{self._ppu_scanline()}). This is a stall guard, not a budget — "
            f"observed progress pushes the deadline out — so a merely slow "
            f"host does not land here. Two things do: a Step request no "
            f"reissue recovered, or a host so oversubscribed the emulation "
            f"thread is not being scheduled at all (measured reachable at 3x "
            f"CPU oversubscription). Check the host's load before the ROM's."
        )
    ...
    def frame_step(self, n: int = 1, controller_index: int = 0,
        ...
        Args:
            ...
            controller_index: Controller port (0 = P1, 1 = P2).
            timeout_s: Initial stall deadline, in wall seconds; scales with
                n by default. NOT a total budget — `_await_step` pushes it
                forward on every scanline the emulator advances, so a
                starved host makes this slower and never makes it fail.
    ...
    def debug_resume(self, clear_input: bool = True, timeout_s: float = 10.0):
        ...
        if clear_input:
            self.set_input(0)
        # Back to this runner's free-run policy — NOT to a hardcoded 60 fps.
        # A hardcoded False here would undo L2 for every test that ever parks
        # (which is most of them), because frame_stepping()'s exit calls this.
        self._apply_free_run_speed()
    ...
    def run_frames(self, n: int):
        ...
        **Prefer :meth:`wait_frames` for anything a test asserts on.** This
        method sleeps n/60 WALL seconds beside a core that advances on its
        own thread, so how many frames it actually buys depends on the host's
        load and on the emulation-speed throttle — under free-run it is ~4x
        what the argument says, and on a parked runner it is ZERO. Every wait
        in the suite is counted in emulated frames for that reason. The one
        deliberate exception is AUDIO: a WAV is a recording of real time, so
        `enable_audio=True` runners stay throttled to 60 fps and keep using
        this method (tests/test_slice_b_audio.py:17-19,
        tests/test_runner_guard.py). `timeout_s=` appears nowhere in `tests/`
        — the three breakpoint sample hunts that still carried wall budgets
        have been converted.
        ...
        throughput work item measured this single change as ~16.7 ms per
        round-trip on the large streaming ROM — the largest single
    ...
    # --- Emulated-frame waits: the de-flake primitives ---------------------
    #
    # ``run_frames`` above bounds a wait in WALL time while the condition it
    # is waiting for advances in EMULATED time. On an idle box the two agree
    # and everything passes; under host load the emulator's frames per wall
    # second drops, the wall budget expires before the predicate flips, and
    # the test goes red with nothing wrong in the ROM. That is the whole of
    # the suite's documented flake class — one mechanism,
    # five sites.
    #
    # These two bound the wait in EMULATED frames instead, read from the
    # PPU's own frame counter (:meth:`ppu_frame_count` — emulated ground
    # truth, and the same counter the deterministic ``frame_step`` path
    # already uses, which is why THAT path has never flaked). Host load then
    # changes only how long a wait takes in wall seconds, never how many
    # frames it covers.
    #
    # The one wall-clock element left is the STALL guard, and it is not a
    # budget: it fires only when the emulated frame counter does not advance
    # AT ALL for ``stall_s`` seconds. That means a wedged emulator or one
    # parked in frame-stepping mode — never a merely slow one. A machine
    # making any progress at all is never cut off, so no amount of host load
    # can turn a passing test red through this path.
    #
    # POLL CADENCE is 1 ms, so a CHEAP predicate is sampled several times per
    # emulated frame and host load only raises that density (the emulated rate
    # falls, the poll rate does not). A predicate that itself costs
    # milliseconds sets its own cadence and can sample less than once a frame
    # — fine for a latched condition, and a caller watching a TRANSIENT one
    # should frame_step instead of polling for it either way. This is not a
    # regression on what it replaced: the `settle` loop it came from looked
    ...
    def _wait_stall_guard(self, cur, last, last_progress, stall_s, detail):
        ...
        return last, last_progress

    def _advance_emulated(self, n: int) -> None:
        """Advance n EMULATED frames in whichever execution mode we are in.

        `wait_frames` free-running, `frame_step` parked. Callers that just
        want "n frames of the ROM's time" should not have to know which,
        and the parked case is the one that used to silently do nothing.
        """
        if n <= 0:
            return
        if self._frame_stepping:
    ...
    def wait_frames(self, n: int, stall_s: float = 30.0) -> int:
        """Block until at least ``n`` EMULATED frames have elapsed.

        The emulated-time replacement for ``run_frames(n)``: that one sleeps
        ``n/60`` wall seconds and hopes the emulator kept up, this one reads
        the PPU frame counter and returns when it has genuinely advanced.

        Args:
            n: Emulated frames to let elapse (0 returns immediately).
            stall_s: Wall seconds of ZERO frame progress that count as a
                wedged/parked emulator. Not a budget — see the block comment.

        Returns:
            Frames actually elapsed (>= n; the 1 ms poll can overshoot by a
            fraction of a frame).
        """
        if n < 0:
            raise ValueError(f"wait_frames needs n >= 0, got {n}")
        start = self.ppu_frame_count()
        if n == 0:
            return 0
        target = start + n
        last, last_progress = start, time.time()
        while True:
            cur = self.ppu_frame_count()
            if cur >= target:
                return cur - start
            last, last_progress = self._wait_stall_guard(
                cur, last, last_progress, stall_s,
                f"wait_frames({n}) [{cur - start} elapsed]")
            time.sleep(self._WAIT_POLL_S)

    def wait_until(self, pred, max_frames: int = 600, what: str = "",
                   stall_s: float = 30.0) -> int:
        """Poll ``pred`` until true; give up after ``max_frames`` EMULATED frames.

        Args:
            pred: Zero-arg callable, polled every millisecond. It should read
                the OUTPUT region the caller cares about (VRAM/OAM/CGRAM/WRAM)
                — this method changes only WHEN the caller looks, never what
                it reads.
            max_frames: Budget, in emulated frames. The whole point: the same
                drive covers the same number of frames on an idle box and on
                a hammered one.
            what: Description used in the TimeoutError.
            stall_s: Frozen-counter guard; see the block comment above.

        Returns:
            Emulated frames elapsed before the predicate flipped. This is a
            determinism handle: identical drives report identical counts run
            to run, so a caller can log it and diff.

        Raises:
            TimeoutError: budget exhausted (message carries the frame count),
                or the emulator froze.
        """
        if max_frames < 0:
            raise ValueError(f"wait_until needs max_frames >= 0, got {max_frames}")
        label = what or "condition"
        start = self.ppu_frame_count()
        last, last_progress = start, time.time()
        while True:
            if pred():
                return self.ppu_frame_count() - start
            cur = self.ppu_frame_count()
            elapsed = cur - start
            if elapsed >= max_frames:
                raise TimeoutError(
                    f"{label}: still false after {elapsed} EMULATED frames "
                    f"(budget {max_frames}). The budget is in emulated "
                    f"frames, not wall time, so host load did not cause "
                    f"this — the condition genuinely did not happen.")
            last, last_progress = self._wait_stall_guard(
                cur, last, last_progress, stall_s,
                f"wait_until({label}) [{elapsed} elapsed]")
            time.sleep(self._WAIT_POLL_S)

    def boot_rom(self, rom_path: str, frames: int = 120,
                 ready=None, ready_what: str = "") -> int:
        """Load a ROM and let it boot for a bounded number of EMULATED frames.

        The replacement for ``load_rom(path, run_seconds=N)``: that one sleeps
        N wall seconds, which is a different amount of boot depending on the
        host's load and on whether the speed throttle is lifted. This gives
        the ROM an exact frame budget.

        Args:
            rom_path: the .sfc to load.
            frames: emulated frames to let elapse when no ``ready`` predicate
                is given (120 == the 2.0 s @ 60 fps the wall-clock callers
                were asking for).
            ready: optional boot-complete predicate. When given, the wait ends
                as soon as it flips and ``frames`` becomes its budget.

                **It must be POWER-ON SAFE.** RAM is random at power-on
                (CLAUDE.md rule 5), so a predicate like "this counter is past
                10" can be TRUE on frame 0 against uninitialised garbage and
                skip the boot entirely — observed while writing
                tests/test_wait_primitives.py, where it returned 0. Use a
                signature the ROM writes (the toy's ``SFOK``, the probes'
                ``SFDB``), not a plausible-looking value in a live variable.
                When no such signature exists, the frame budget is the honest
                answer, which is why it is the default.
            ready_what: description for ``ready``'s TimeoutError.

        Returns:
            Emulated frames the boot took.
        """
        self.load_rom(rom_path, run_seconds=0.0)
        if ready is None:
            return self.wait_frames(frames)
        return self.wait_until(ready, max_frames=frames,
                               what=ready_what or f"{rom_path} boot")

    #: Emulated frames of the boot that `boot_to_frame` walks parked rather
    #: than free-running. It only has to cover the free-run overshoot
    #: (`wait_frames` returns ">= n", and the break lands some way past
    #: that): measured on this box, a fixed free-run budget landed on
    #: 90/91/92 under contention against a flat 91 when idle. 20 is ~7x that
    #: spread and still under a third of a second of stepping.
    _BOOT_PARK_MARGIN = 20

    def boot_to_frame(self, rom_path: str, frame: int,
                      margin: Optional[int] = None,
                      uninit_detection: bool = False):
        """Boot a ROM and land PARKED on an EXACT absolute emulated frame.

        WHY THIS EXISTS, and why `boot_rom` is not enough. `boot_rom(rom,
        frames=N)` bounds the boot in emulated frames, which is already
        immune to how FAST the host is — but it lands on ">= N", not on N.
        `wait_frames`' 1 ms poll can miss a frame boundary and the gap
        between it returning and a subsequent break landing is a host-load
        question. MEASURED on this box: a fixed free-run budget landed on
        frame 90, 91 or 92 under contention against a flat 91 when idle.

        Two frames is nothing to a test that reads a settled value, and it is
        the whole ballgame to a test that reads the PICTURE. The microzero
        race render's near rows sit on the black/white start-finish checker,
        ...
        So: free-run most of the boot (which is the fast part, and where the
        slop is harmless), then park and STEP the last `margin` frames, which
        is exact. Every host then renders the same picture.

        Stepping cannot go backwards, so an overshoot is an ASSERT rather
        than a silent clamp — a clamp would quietly hand back the
        host-dependent behaviour this exists to remove.

        Args:
            rom_path: the .sfc to load.
            frame: the ABSOLUTE PPU frame number to land on.
            margin: emulated frames to walk parked (default
                :data:`_BOOT_PARK_MARGIN`). Raise it if a loaded host still
                overshoots; the cost is linear stepping time.
            uninit_detection: arm the per-address access counters from
                power-on (:meth:`load_rom_with_uninit_detection`) so
                :meth:`assert_no_uninitialized_reads` can be called after.

        Returns:
            The frame landed on, which is exactly `frame`.

        Raises:
            AssertionError: the free-run phase overshot `frame`.
        """
        m = self._BOOT_PARK_MARGIN if margin is None else margin
        if frame < m:
            raise ValueError(
                f"boot_to_frame({frame}) with margin {m}: the whole boot "
                f"would be stepped. Pass margin=<{frame} or raise the frame.")
        if uninit_detection:
            self.load_rom_with_uninit_detection(rom_path, frames=frame - m)
        else:
            self.load_rom(rom_path, run_seconds=0.0)
            self.wait_frames(frame - m)
        self.debug_break()
        landed = self.ppu_frame_count()
        assert landed < frame, (
            f"boot_to_frame: the free-run boot overshot to frame {landed}, "
            f"past the {frame} normalisation point — raise `frame` or lower "
            f"`margin`. Do NOT drop this assert or clamp it: a clamp puts the "
            f"host back in charge of which frame you photographed.")
        self.frame_step(frame - landed)
        return frame
    ...
    def await_streaming_idle(self, timeout_frames: int = 60) -> bool:
        ...
        ----------------------
        The a later stage streaming engine
        ...
        A streaming-correctness test hit this footgun. Its poll loop was
        inlined in the test
...
class Assembler:
    ...
    Supports three linker configurations:
      - lorom.cfg: 32KB single-bank LoROM (default, the early tests)
    ...
    def __init__(
        ...
        """
        # Auto-detection is DISABLED in SuperForge and this is deliberate.
        #
        # As vendored, this resolved `parent.parent.parent / "infrastructure" /
        # "rom_template"` — one level too high for where this file sits now, so
        # it points ABOVE the repo root, at a directory that does not exist here
        # at all (there is no `infrastructure/`; the ROM assets are
        # `vendor/rom/`).
        #
        # Pointing it at vendor/rom instead would not fix it either: this class
        # names configs `<rom_type>.cfg` (lorom.cfg / hirom.cfg), while
        # vendor/rom uses size-suffixed names (lorom_32k.cfg, lorom_512k.cfg,
        # lorom_64k.cfg). There is no correct default, so demand an explicit one
        # rather than fail later with a confusing FileNotFoundError pointing
        # somewhere outside the repo.
        #
        # Nothing in this repo constructs an Assembler today; it is kept
        # against the day something does. If you are the first caller, pass the
        # paths explicitly.
        if template_dir is None:
            raise ValueError(
                "Assembler(template_dir=...) is required in SuperForge.\n"
                "This vendored helper's auto-detection assumes an "
                "`infrastructure/rom_template` layout, which does not exist "
                "here.\n"
                "Pass template_dir explicitly, and linker_cfg too if your "
                "config is not named '<rom_type>.cfg' — vendor/rom/ uses "
                "lorom_32k.cfg / lorom_64k.cfg / lorom_512k.cfg."
            )
